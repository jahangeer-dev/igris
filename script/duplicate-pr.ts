...
import { createOpencode } from "@igris-ai/sdk"
...
`)
  ...
  const igris = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await igris.client.session.create()
    const result = await igris.client.session
      .prompt({
