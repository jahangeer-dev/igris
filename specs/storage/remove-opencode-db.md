# Remove `packages/igris/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/igris/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the igris legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/igris/src/storage/db.ts` are concentrated in 22 source files:

- `packages/igris/src/account/repo.ts`
- `packages/igris/src/cli/cmd/db.ts`
- `packages/igris/src/cli/cmd/import.ts`
- `packages/igris/src/cli/cmd/stats.ts`
- `packages/igris/src/control-plane/workspace.ts`
- `packages/igris/src/index.ts`
- `packages/igris/src/node.ts`
- `packages/igris/src/permission/index.ts`
- `packages/igris/src/project/project.ts`
- `packages/igris/src/server/projectors.ts`
- `packages/igris/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/igris/src/server/shared/fence.ts`
- `packages/igris/src/session/message-v2.ts`
- `packages/igris/src/session/projectors.ts`
- `packages/igris/src/session/prompt.ts`
- `packages/igris/src/session/session.ts`
- `packages/igris/src/session/todo.ts`
- `packages/igris/src/share/share-next.ts`
- `packages/igris/src/storage/db.ts`
- `packages/igris/src/sync/index.ts`
- `packages/igris/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy igris database wrapper; `packages/igris/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the igris wrapper.
...
Status: Completed. `SyncEvent` and the igris projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy igris database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy igris database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy igris database wrapper.
...
All migration groups are complete or superseded. `packages/igris/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No igris data-migration group remains.

The previous igris `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate igris data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/igris`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/igris/src`
- `bun typecheck` from `packages/igris`
- Relevant package tests from `packages/igris`, not the repo root
