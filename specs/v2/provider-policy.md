...
Both `action` and `resource` use igris's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.igris` files is intentionally deferred until `.igris` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether igris permits an operation through its managed execution paths.
