...
The igris server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@igris-ai/llm` tool loop after replacing its
...
- plugins get global "igris" instance like in that post i showed
- igris instance has stuff like `igris.session.prompt()` or
  `igris.tool.register({...})`
...
- decide whether to preserve deprecated `@igris-ai/llm` orchestration exports
