...
Move the canonical Igris terminal application from
`packages/igris/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/igris`,
`packages/cli`, or `@igris-ai/core`.

The SDK is the TUI's Igris boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/igris`.
- Do not replace private `packages/igris` imports with `@igris-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@igris-ai/tui`.
...
- Igris domain data displayed by the TUI
...
```text
packages/igris/src/cli/cmd/tui
```

Its private dependency on `packages/igris` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/igris/src/*`.
...
Status: Completed. The private `@igris-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@igris-ai/tui`.
...
- The package has no dependency on `igris`, `@igris-ai/cli`, or
  `@igris-ai/core`.
...
primitives now live in `@igris-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@igris-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Igris globals or process state;
...
Replace process-global Igris state with resolved TUI inputs.
...
`@igris-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/igris` initially.
...
canonical in `@igris-ai/tui`. Configured references resolve through the new
...
adapters rather than Igris domain implementations.

Make the SDK the only Igris domain boundary used by the TUI.
...
- Domain-facing TUI code imports Igris data and operations only from
  `@igris-ai/sdk`.
...
`@igris-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/igris`.
...
- The package has no imports from `packages/igris`, `packages/cli`, or
  `@igris-ai/core`.
...
public `@igris-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/igris`.
- Keep the legacy embedded worker and server startup in `packages/igris`.
- Change those adapters to load config, create transport inputs, and call the
  public `@igris-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/igris`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@igris-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@igris-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Igris domain state.
...
cd packages/igris && bun typecheck
...
rg '@igris-ai/core|packages/igris|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/igris/src packages/igris/test
```
...
- Build the current-platform `packages/igris` binary.
