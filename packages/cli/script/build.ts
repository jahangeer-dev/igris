...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      ...
      IGRIS_CLI_NAME: `'${binary}'`,
      IGRIS_MODELS_DEV: modelsData,
      IGRIS_CHANNEL: `'${Script.channel}'`,
      IGRIS_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
    ...
    JSON.stringify(
      {
        ...
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/igris.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
