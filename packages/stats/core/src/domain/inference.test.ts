...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve igris route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "igris")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "igris")).toBe("openai")
    expect(statProvider("big-pickle", "", "igris")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "igris"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep igris as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "igris"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "igris"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep igris or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "igris"), country: "US" })).toMatchObject([
