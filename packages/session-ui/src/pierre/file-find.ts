...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("igris-find")
  api.delete("igris-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("igris-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("igris-find", new Highlight(...rest))
    return true
  }
