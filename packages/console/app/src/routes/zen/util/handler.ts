...
import { and, Database, eq, isNull, lt, or, sql } from "@igris-ai/console-core/drizzle/index.js"
import { KeyTable } from "@igris-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@igris-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@igris-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@igris-ai/console-core/util/date.js"
import { Identifier } from "@igris-ai/console-core/identifier.js"
import { Billing } from "@igris-ai/console-core/billing.js"
import { Actor } from "@igris-ai/console-core/actor.js"
import { WorkspaceTable } from "@igris-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@igris-ai/console-core/model.js"
import { Subscription } from "@igris-ai/console-core/subscription.js"
import { BlackData } from "@igris-ai/console-core/black.js"
import { UserTable } from "@igris-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@igris-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@igris-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@igris-ai/console-core/lite.js"
import { Resource } from "@igris-ai/console-resource"
...
import { Workspace } from "@igris-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-igris-session") ?? ""
    const requestId = input.request.headers.get("x-igris-request") ?? ""
    const ocClient = input.request.headers.get("x-igris-client") ?? ""
    const projectId = input.request.headers.get("x-igris-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://igris.ai/workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-igris-request")
            headers.delete("x-igris-session")
            headers.delete("x-igris-project")
            headers.delete("x-igris-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-igris-endpoint-id")
        const resEndpointModelId = res.headers.get("x-igris-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://igris.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://igris.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://igris.ai/workspace/${authInfo.workspaceID}/members`
