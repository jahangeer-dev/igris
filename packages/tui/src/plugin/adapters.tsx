import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@igris-ai/plugin/tui"
...
import type { useIgrisKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useIgrisKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getIgrisModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getIgrisModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
