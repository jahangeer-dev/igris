...
export const IGRIS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const IGRIS_MODE_KEY = "igris.mode"

export const IgrisKeymapProvider = KeymapProvider
export const useIgrisKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type IgrisModeStack = ReturnType<typeof createIgrisModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, IgrisModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createIgrisModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(IGRIS_MODE_KEY, IGRIS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(IGRIS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(IGRIS_MODE_KEY, stack.at(-1)?.mode ?? IGRIS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? IGRIS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(IGRIS_MODE_KEY, undefined)
...
export function useIgrisModeStack() {
  return getIgrisModeStack(useIgrisKeymap())
}

export function getIgrisModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Igris mode stack is not registered for this keymap")
  return value
}
...
export function registerIgrisKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createIgrisModeStack(keymap)
