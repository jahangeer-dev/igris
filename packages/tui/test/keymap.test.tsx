...
import { getIgrisModeStack, IGRIS_BASE_MODE, IgrisKeymapProvider, registerIgrisKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerIgrisKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when igris mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getIgrisModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getIgrisModeStack(keymap).push("autocomplete")
