...
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\igris\log`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `igris --log-level DEBUG`.

---

## Storage

igris stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop runs a local OpenCode server (the `igris-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (or `~/.config/igris/igris.json`)
- **macOS/Linux** (older installs): `~/.local/share/igris/igris.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Global plugins**
  ...
  - **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\igris\plugins`
...
- **Linux**: delete `~/.cache/igris` (or run `rm -rf ~/.cache/igris`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\igris`
...
If your `igris.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
- `igris.global.dat` and `igris.workspace.*.dat` (UI state like recent servers/projects)
...
3. Ensure you have the latest version with `igris upgrade`
...
To figure out what models you have access to, run `igris models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.local\share\igris`
...
If you encounter API call errors, this may be due to outdated provider packages. igris dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   On Windows, press `WIN+R` and delete: `%USERPROFILE%\.cache\igris`

2. Restart igris to reinstall the latest provider packages

This will force igris to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
igris will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
