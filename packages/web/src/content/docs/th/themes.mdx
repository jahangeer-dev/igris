...
ตามค่าเริ่มต้น OpenCode จะใช้ธีม `igris` ของเราเอง
...
2. **ไดเรกทอรีกำหนดค่าผู้ใช้** - กำหนดใน `~/.config/igris/themes/*.json` หรือ `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **ไดเรกทอรีรากของโครงการ** - กำหนดไว้ใน `<project-root>/.igris/themes/*.json`
4. **ไดเร็กทอรีการทำงานปัจจุบัน** - กำหนดใน `./.igris/themes/*.json`
