...
1. **Remote config** (from `.well-known/igris`) - organizational defaults
2. **Global config** (`~/.config/igris/igris.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`igris.json` in project) - project-specific settings
5. **`.igris` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/igris/` on macOS) - admin-controlled
...
:::note
The `.igris` and `~/.config/igris` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/igris` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global OpenCode config in `~/.config/igris/igris.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/igris/tui.json`.
...
Add `igris.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
modes, and plugins just like the standard `.igris` directory, and should
...
The custom directory is loaded after the global config and `.igris` directories, so it **can override** their settings.
...
Drop an `igris.json` or `igris.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/igris/` |
| Linux    | `/etc/igris/`                         |
| Windows  | `%ProgramData%\igris`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, OpenCode reads managed preferences from the `ai.igris.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

OpenCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.igris.managed.plist`
2. `/Library/Managed Preferences/ai.igris.managed.plist`

The plist keys map directly to `igris.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.igris.managed` PayloadType. The OpenCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.igris.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.igris.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.igris</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`igris.ai/config.json`**](https://igris.ai/config.json).

TUI config uses [**`igris.ai/tui.json`**](https://igris.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `igris.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `igris serve` and `igris web` commands through the `server` option.
...
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `igris.local`. Useful for running multiple instances on the same network.
...
You can also define agents using markdown files in `~/.config/igris/agents/` or `.igris/agents/`. [Learn more here](/docs/agents).
...
This setting applies across all interfaces: TUI, CLI (`igris run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/igris/commands/` or `.igris/commands/`. [Learn more here](/docs/commands).
...
By default, igris **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
Place plugin files in `.igris/plugins/` or `~/.config/igris/plugins/`. You can also load plugins from npm through the `plugin` option.
