...
igris ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\igris\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `igris --log-level DEBUG`.

---

## Depolama

igris, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\igris` yapistirin
...
igris Desktop arka planda yerel bir igris sunucusu (`igris-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `igris` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (veya `~/.config/igris/igris.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/igris/igris.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\igris\igris.jsonc` yapistirin
...
igris diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/igris/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\igris\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), igris'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. igris Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/igris`
- **Linux**: `~/.cache/igris` dizinini silin (veya `rm -rf ~/.cache/igris` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\igris` yapistirin

3. igris Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

igris Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`igris.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta igris Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, igris ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

igris Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda igris icin bildirimler etkinse
...
1. igris Desktop'u kapatin
2. Su dosyalari bulun ve silin (igris Desktop uygulama veri dizininde yer alirlar):

- `igris.settings.dat` (desktop varsayilan sunucu URL'si)
- `igris.global.dat` ve `igris.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
igris ile ilgili bir sorun yasiyorsaniz:
...
3. `igris upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `igris models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\igris`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. igris, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\igris`

2. En guncel provider paketlerini yeniden kurmak icin igris'u yeniden baslatin

Bu, igris'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
igris, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
