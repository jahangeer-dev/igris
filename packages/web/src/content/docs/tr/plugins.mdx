...
- `.igris/plugins/` - Proje düzeyinde eklentiler
- `~/.config/igris/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/igris/node_modules/`'da önbelleğe alınır.
...
3. Global eklenti dizini (`~/.config/igris/plugins/`)
4. Proje eklenti dizini (`.igris/plugins/`)
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://igris.ai/docs/sdk) bakın.
