...
_Model Bağlam Protokolü_ veya MCP kullanarak igris'a harici araçlar ekleyebilirsiniz. igris hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [igris Config](https://igris.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.

```jsonc title="igris.jsonc" {6}
{
...
Kuruluşlar, `.well-known/igris` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
igris, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde igris şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa igris, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `igris mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
```bash
igris mcp auth my-oauth-server
```
...
```bash
igris mcp logout my-oauth-server
```

`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından igris, belirteçleri `~/.local/share/igris/mcp-auth.json`'de güvenli bir şekilde saklar.
...
igris mcp debug my-oauth-server
```
...
MCP'leriniz yerleşik araçların yanı sıra igris'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi igris yapılandırması aracılığıyla yönetebilirsiniz.
...
```json title="igris.json" {11, 14-18}
{
  "$schema": "https://igris.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
igris mcp auth sentry
```

Bu, OAuth akışını tamamlamak ve igris'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
