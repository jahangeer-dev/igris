...
igris ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak igris kendi `igris` temasını kullanır.
...
igris birden fazla yerleşik temayla gelir.
...
- igris'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
igris, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/igris/themes/*.json` veya `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.igris/themes/*.json`
4. **Geçerli çalışma dizini** - `./.igris/themes/*.json`
