---
title: Sağlayıcılar
description: igris'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

igris, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı igris yapılandırmanızda yapılandırın.
...
`~/.local/share/igris/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları igris'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenCode Zen, igris ekibi tarafından sağlanan modellerin bir listesidir.
igris ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenCode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, igris'u seçin ve [igris.ai/auth](https://igris.ai/auth)'ye gidin.
...
igris'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenCode Go

OpenCode Go, igris ile iyi çalıştığı test edilmiş ve doğrulanmış, igris ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenCode Go`'yu seçin ve [igris.ai/auth](https://igris.ai/zen) adresine gidin.
...
Amazon Bedrock'u igris ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   igris'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `igris.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi igris'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
igris'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   igris'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
to store token in igris auth storage.
:::

##### Kendi Kendine Barındırılan GitLab

:::note[uyumluluk notu]
igris, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. igris'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`igris.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth) ana sayfasında.

##### Yapılandırma

`igris.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi igris'la kullanmak için:
...
Google Vertex AI'yi igris ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları igris çalıştırılırken ayarlayın.
...
Helicone'un igris aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her igris görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her igris görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
igris'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla igris'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla igris'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini igris için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/igris)'a bakın.
:::
...
Ollama Cloud'u igris ile kullanmak için:
...
3. igris'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: igris'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenCode Zen, igris ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. igris API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   igris yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca igris yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri igris yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı igris ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
3. Sağlayıcı için API anahtarınızı girin.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in igris.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Proje dizininizde `igris.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları igris'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `igris auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için igris yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, igris yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
