---
title: CLI
description: igris CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

igris CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
igris
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, igris ile programlı olarak etkileşim kurmanıza olanak tanır.
...
igris terminal kullanıcı arayüzünü başlatır.
...
igris CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

igris için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir igris arka uç sunucusuna bir terminal ekleyin.

```bash
igris attach [url]
```

Bu, TUI öğesinin uzak bir igris arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `igris`) |
...
igris, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `igris auth login` kullanabilirsiniz. Bu `~/.local/share/igris/auth.json`'de saklanıyor.

```bash
igris auth login
```

igris başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları igris'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek igris'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `igris serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir igris sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `igris`) |
...
API erişimi için headless bir igris sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
igris serve
```

Bu, TUI arayüzü olmadan igris işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `igris` şeklindedir).
...
Tüm igris oturumlarını listeleyin.
...
igris oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya igris paylaşımından URL oturum verilerini içe aktarın.

```bash
igris import <file>
```

Yerel bir dosyadan veya igris paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir igris sunucusu başlatın.

```bash
igris web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla igris'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `igris` şeklindedir).
...
igris'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
igris'u en son sürüme veya belirli bir sürüme günceller.
...
igris CLI aşağıdaki global bayrakları destekler.
...
igris ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `igris`) |
