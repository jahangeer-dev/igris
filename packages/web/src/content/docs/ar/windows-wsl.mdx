...
1. **شغّل `igris web` من terminal داخل WSL** بدلًا من PowerShell:
...
تشغيل `igris web` من WSL يضمن وصولًا صحيحًا لنظام الملفات وتكاملًا أفضل مع terminal، مع بقائه متاحًا من متصفح Windows.
...
- إعدادات OpenCode وجلساته تُخزَّن داخل بيئة WSL في `~/.local/share/igris/`
