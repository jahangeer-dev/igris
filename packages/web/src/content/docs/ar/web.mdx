...
:::tip[مستخدمو Windows]
لأفضل تجربة، شغّل `igris web` من [WSL](/docs/windows-wsl) بدلا من PowerShell. يضمن ذلك وصولا صحيحا إلى نظام الملفات وتكاملا أفضل مع terminal.
:::
...
يضبط ذلك تلقائيا اسم المضيف إلى `0.0.0.0` ويعلن عن الخادم باسم `igris.local`.
...
اسم المستخدم الافتراضي هو `igris`، ويمكن تغييره عبر `OPENCODE_SERVER_USERNAME`.
...
يمكنك أيضا ضبط إعدادات الخادم داخل ملف الإعدادات `igris.json`:
