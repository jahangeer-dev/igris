...
- `.igris/plugins/` - إضافات على مستوى المشروع
- `~/.config/igris/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/igris/node_modules/`.
...
1. إعدادات عامة (`~/.config/igris/igris.json`)
2. إعدادات المشروع (`igris.json`)
3. دليل الإضافات العام (`~/.config/igris/plugins/`)
4. دليل إضافات المشروع (`.igris/plugins/`)
...
- `client`: عميل SDK لـ igris للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع igris.
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى igris:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ igris استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ igris إلى جانب الأدوات المضمنة.
...
المستويات: `debug`, `info`, `warn`, `error`. راجع [توثيق SDK](https://igris.ai/docs/sdk) للتفاصيل.
