---
title: الخادم
description: تفاعل مع خادم igris عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `igris serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل igris استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `igris.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `igris`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `igris serve` و `igris web`.
...
عند تشغيل `igris` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم igris للتفاعل مع igris برمجيا.
:::

تتيح هذه البنية لـ igris دعم عدة عملاء وتمكّنك من التفاعل مع igris برمجيا.

يمكنك تشغيل `igris serve` لبدء خادم مستقل. إذا كانت واجهة igris في terminal (TUI)
قيد التشغيل، فسيبدأ `igris serve` خادما جديدا.
...
يعرض خادم igris واجهات API التالية.
