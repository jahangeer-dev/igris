...
على سبيل المثال، إذا كان الإعداد العام يضبط `theme: "igris"` و`autoupdate: true`، وكان إعداد المشروع يضبط `model: "anthropic/claude-sonnet-4-5"`، فستتضمن الإعدادات النهائية الخيارات الثلاثة جميعها.
...
1. **الإعدادات البعيدة** (من `.well-known/igris`) - الإعدادات الافتراضية على مستوى المؤسسة
2. **الإعدادات العامة** (`~/.config/igris/igris.json`) - تفضيلات المستخدم
3. **إعدادات مخصصة** (`OPENCODE_CONFIG` env var) - تجاوزات مخصصة
4. **إعدادات المشروع** (`igris.json` داخل المشروع) - إعدادات خاصة بالمشروع
5. **أدلة `.igris`** - الوكلاء، الأوامر، الإضافات
...
:::note
يستخدم الدليلان `.igris` و`~/.config/igris` **أسماء جمع** للأدلة الفرعية: `agents/` و`commands/` و`modes/` و`plugins/` و`skills/` و`tools/` و`themes/`. كما تُدعَم أسماء المفرد (مثل `agent/`) للتوافق مع الإصدارات السابقة.
:::
...
يمكن للمؤسسات توفير إعدادات افتراضية عبر نقطة النهاية `.well-known/igris`. يتم جلب ذلك تلقائيًا عندما تقوم بالمصادقة مع مزوّد يدعم هذه الميزة.
...
ضع إعدادات OpenCode العامة في `~/.config/igris/igris.json`. استخدم الإعدادات العامة للتفضيلات على مستوى المستخدم مثل السمات، والمزوّدين، أو اختصارات المفاتيح.
...
أضف `igris.json` في جذر مشروعك. تمتلك إعدادات المشروع أعلى أولوية بين ملفات الإعدادات القياسية، إذ تتجاوز الإعدادات العامة والبعيدة معًا.
...
الدليل القياسي `.igris`، ويجب أن يتبع نفس البنية.
...
يُحمَّل الدليل المخصص بعد الإعدادات العامة وأدلة `.igris`، لذلك **يمكنه تجاوز** إعداداتها.

---

## المخطط

يحتوي ملف الإعدادات على مخطط مُعرَّف في [**`igris.ai/config.json`**](https://igris.ai/config.json).
...
يمكنك ضبط إعدادات الخادم لأوامر `igris serve` و`igris web` عبر الخيار `server`.
...
- `mdnsDomain` - اسم نطاق مخصص لخدمة mDNS. القيمة الافتراضية هي `igris.local`. مفيد لتشغيل عدة نُسخ على نفس الشبكة.
...
يمكنك أيضًا تعريف الوكلاء باستخدام ملفات markdown في `~/.config/igris/agents/` أو `.igris/agents/`. [تعرف على المزيد هنا](/docs/agents).
...
ينطبق هذا الإعداد على جميع الواجهات: TUI وCLI (`igris run`) وتطبيق سطح المكتب وGitHub Action.
...
يمكنك أيضًا تعريف الأوامر باستخدام ملفات markdown في `~/.config/igris/commands/` أو `.igris/commands/`. [تعرف على المزيد هنا](/docs/commands).
...
افتراضيًا يسمح igris **بجميع العمليات** دون الحاجة إلى موافقة صريحة. يمكنك تغيير ذلك عبر الخيار `permission`.
...
ضع ملفات الإضافات في `.igris/plugins/` أو `~/.config/igris/plugins/`. يمكنك أيضًا تحميل إضافات من npm عبر الخيار `plugin`.
