...
افتراضيًا، يستخدم OpenCode سمتنا `igris`.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/igris/themes/*.json` أو `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.igris/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.igris/themes/*.json`
