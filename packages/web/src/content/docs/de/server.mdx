---
title: Server
description: Nutze den igris-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `igris serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein igris-Client nutzen kann.
...
Der Benutzername ist standardmaessig `igris`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `igris serve` und `igris web`.
...
Wenn du `igris` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den igris-Server, um igris programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `igris serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `igris serve` trotzdem eine neue Serverinstanz.
...
Der igris-Server stellt folgende APIs bereit.
