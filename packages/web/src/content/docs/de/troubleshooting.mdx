...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\igris\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `igris --log-level DEBUG`.

---

## Speicher

igris speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop startet im Hintergrund einen lokalen OpenCode-Server (`igris-cli`-Sidecar).
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (oder `~/.config/igris/igris.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/igris/igris.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Globale Plugins**
  ...
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\igris\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/igris`
- **Linux**: loesche `~/.cache/igris` (oder fuehre aus: `rm -rf ~/.cache/igris`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\igris`
...
Wenn deine `igris.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
...
- `igris.global.dat` und `igris.workspace.*.dat` (UI state like recent servers/projects)
...
3. Stelle sicher, dass du die neueste Version hast: `igris upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `igris models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\igris`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. igris installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\igris`

2. Starte igris neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt igris, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
igris erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
