---
title: Regeln
description: Lege eigene Anweisungen fuer igris fest.
---

Du kannst igris ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in igris den Befehl `/init` aus.
...
So kann igris sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht igris sie zu erweitern.
...
igris kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/igris/AGENTS.md` speichern.
Diese gelten in allen igris-Sitzungen.
...
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/igris/AGENTS.md` existiert)
...
Beim Start sucht igris in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/igris/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `igris.json` oder in `~/.config/igris/igris.json` angeben.
...
igris parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit igris.json

Empfohlen ist das `instructions`-Feld in der `igris.json`:
...
Du kannst igris explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `igris.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
