...
Wenn Ihre globale Konfiguration beispielsweise `theme: "igris"` und `autoupdate: true` und Ihre Projektkonfiguration `model: "anthropic/claude-sonnet-4-5"` festlegt, umfasst die endgültige Konfiguration alle drei Einstellungen.
...
1. **Remote-Konfiguration** (von `.well-known/igris`) – Organisationsstandards
2. **Globale Konfiguration** (`~/.config/igris/igris.json`) – Benutzereinstellungen
3. **Benutzerdefinierte Konfiguration** (`OPENCODE_CONFIG` env var) – benutzerdefinierte Überschreibungen
4. **Projektkonfiguration** (`igris.json` im Projekt) – projektspezifische Einstellungen
5. **`.igris` Verzeichnisse** – Agenten, Befehle, Plugins
...
:::note
Die Verzeichnisse `.igris` und `~/.config/igris` verwenden **Pluralnamen** für Unterverzeichnisse: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` und `themes/`. Aus Gründen der Abwärtskompatibilität werden auch einzelne Namen (z. B. `agent/`) unterstützt.
:::
...
Organisationen können die Standardkonfiguration über den Endpunkt `.well-known/igris` bereitstellen. Dieser wird automatisch abgerufen, wenn Sie sich bei einem Anbieter authentifizieren, der dies unterstützt.
...
Platzieren Sie Ihre globale OpenCode-Konfiguration in `~/.config/igris/igris.json`. Verwenden Sie die globale Konfiguration für benutzerweite Einstellungen wie Themen, Anbieter oder Tastenkombinationen.
...
Fügen Sie `igris.json` in Ihrem Projektstamm hinzu. Die Projektkonfiguration hat unter den Standardkonfigurationsdateien die höchste Priorität – sie überschreibt sowohl globale als auch Remote-Konfigurationen.
...
Modi und Plugins genau wie das Standardverzeichnis `.igris` und sollten
...
Das benutzerdefinierte Verzeichnis wird nach den Verzeichnissen global config und `.igris` geladen, sodass es deren Einstellungen **überschreiben** kann.

---

## Schema

Die Konfigurationsdatei verfügt über ein Schema, das in [**`igris.ai/config.json`**](https://igris.ai/config.json) definiert ist.
...
Sie können Servereinstellungen für die Befehle `igris serve` und `igris web` über die Option `server` konfigurieren.
...
- `mdnsDomain` – Benutzerdefinierter Domänenname für den mDNS-Dienst. Standardmäßig ist `igris.local`. Nützlich für die Ausführung mehrerer Instanzen im selben Netzwerk.
...
Sie können Agenten auch mithilfe von Markdown-Dateien in `~/.config/igris/agents/` oder `.igris/agents/` definieren. [Mehr erfahren](/docs/agents).
...
Diese Einstellung gilt für alle Schnittstellen: TUI, CLI (`igris run`), Desktop-App und GitHub Aktion.
...
Sie können Befehle auch mithilfe von Markdown-Dateien in `~/.config/igris/commands/` oder `.igris/commands/` definieren. [Mehr erfahren](/docs/commands).
...
Platzieren Sie Ihre Plugin-Dateien in `.igris/plugins/` oder `~/.config/igris/plugins/`. Sie können Plugins auch über die Option `plugin` von npm laden.
