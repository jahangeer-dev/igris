...
1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie OpenCode aus und gehen Sie zu [igris.ai/auth](https://igris.ai/auth).
...
1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `OpenCode Go` und gehen Sie zu [igris.ai/auth](https://igris.ai/zen).
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `igris.json`:
...
Sie können igris so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
`igris.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `igris.json`:
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session)-Plugin, um jede OpenCode-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
:::tip
Ollama kann sich automatisch für OpenCode konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/igris).
:::
...
4. Erstellen oder aktualisieren Sie Ihre `igris.json`-Datei in Ihrem Projektverzeichnis:
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `igris auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
