...
- `.igris/plugins/` – Plugins auf Projektebene
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/igris/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/igris/igris.json`)
2. Projektkonfiguration (`igris.json`)
3. Globales Plugin-Verzeichnis (`~/.config/igris/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.igris/plugins/`)
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://igris.ai/docs/sdk).
