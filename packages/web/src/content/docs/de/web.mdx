...
:::tip[Windows-Nutzer]
Um die beste Erfahrung zu erzielen, führen Sie `igris web` über [WSL](/docs/windows-wsl) und nicht über PowerShell aus. Dies gewährleistet den ordnungsgemäßen Zugriff auf das Dateisystem und die Terminalintegration.
:::
...
Dadurch wird der Hostname automatisch auf `0.0.0.0` gesetzt und der Server als `igris.local` angekündigt.
...
Der Benutzername ist standardmäßig `igris`, kann aber mit `OPENCODE_SERVER_USERNAME` geändert werden.
...
Sie können Servereinstellungen auch in Ihrer `igris.json`-Konfigurationsdatei konfigurieren:
