...
Pour une expérience optimale, exécutez `igris web` à partir de [WSL](/docs/windows-wsl) plutôt que de PowerShell. Cela garantit un accès correct au système de fichiers et une intégration du terminal.
:::
...
Cela définit automatiquement le nom d'hôte sur `0.0.0.0` et annonce le serveur comme `igris.local`.
...
Le nom d'utilisateur par défaut est `igris` mais peut être modifié avec `OPENCODE_SERVER_USERNAME`.
...
Vous pouvez également configurer les paramètres du serveur dans votre fichier de configuration `igris.json` :
