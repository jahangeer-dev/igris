---
title: Serveur
description: Interagissez avec le serveur igris via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `igris serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client igris peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `igris.local` |
...
Définissez `OPENCODE_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `igris`, ou définissez `OPENCODE_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `igris serve` et à `igris web`.
...
Lorsque vous exécutez `igris`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur igris pour interagir avec igris par programmation.
:::

Cette architecture permet à igris de prendre en charge plusieurs clients et vous permet d'interagir avec igris par programme.

Vous pouvez exécuter `igris serve` pour démarrer un serveur autonome. Si vous avez le
igris TUI en cours d'exécution, `igris serve` démarrera un nouveau serveur.
...
Le serveur igris expose les API suivantes.
