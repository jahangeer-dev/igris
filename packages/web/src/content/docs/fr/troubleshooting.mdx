...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\igris\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `igris --log-level DEBUG`.

---

## Stockage

igris stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/igris/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop exécute un serveur OpenCode local (le side-car `igris-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/igris/igris.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Plugins mondiaux**
  ...
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\igris\plugins`
...
- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/igris`
- **Linux** : supprimez `~/.cache/igris` (ou exécutez `rm -rf ~/.cache/igris`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\igris`
...
Si votre `igris.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
- `igris.settings.dat` (serveur de bureau par défaut URL)
- `igris.global.dat` et `igris.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
3. Assurez-vous d'avoir la dernière version avec `igris upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `igris models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\igris`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. igris installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\igris`

2. Redémarrez igris pour réinstaller les derniers packages du fournisseur

Cela forcera igris à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
igris détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
