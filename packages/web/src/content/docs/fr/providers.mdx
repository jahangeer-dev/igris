...
en `~/.local/share/igris/auth.json`.
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez igris et dirigez-vous vers [igris.ai/auth](https://igris.ai/auth).
...
1. Exécutez la commande `/connect` dans le TUI, sélectionnez `OpenCode Go` et rendez-vous sur [igris.ai/auth](https://igris.ai/zen).
...
Définissez l'une de ces variables d'environnement lors de l'exécution de igris :
...
Pour une configuration spécifique au projet ou persistante, utilisez `igris.json` :
...
Vous pouvez configurer igris pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que igris fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration igris.
...
pour stocker le jeton dans le stockage d'authentification igris.
:::
...
Fichier `igris.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth).

##### Configuration

Personnalisez via `igris.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec igris :
...
Définissez-les lors de l'exécution de igris.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via igris, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) pour enregistrer automatiquement chaque conversation OpenCode en tant que session dans Helicone.
...
Vous pouvez configurer igris pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer igris pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer igris pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour OpenCode. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/igris) pour plus de détails.
:::
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration igris.
...
5. Vous pouvez également les personnaliser via votre configuration igris. Voici un exemple de spécification d'un fournisseur
...
Vous pouvez également personnaliser les modèles via votre configuration igris. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec igris. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `igris.json` dans le répertoire de votre projet :
...
1. **Vérifiez la configuration de l'authentification** : exécutez `igris auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration igris et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration igris.
