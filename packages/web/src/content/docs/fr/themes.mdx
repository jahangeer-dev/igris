...
Par défaut, OpenCode utilise notre propre thème `igris`.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/igris/themes/*.json` ou `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.igris/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.igris/themes/*.json`
