...
OpenCode s'intègre à VS Code, Cursor ou tout IDE prenant en charge un terminal. Exécutez simplement `igris` dans le terminal pour commencer.
...
3. Exécutez `igris` - l'extension s'installe automatiquement
...
- Assurez-vous que vous exécutez `igris` dans le terminal intégré.
