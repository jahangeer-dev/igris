...
1. **Configuration distante** (à partir de `.well-known/igris`) - paramètres par défaut de l'organisation
2. **Configuration globale** (`~/.config/igris/igris.json`) - préférences utilisateur
3. **Configuration personnalisée** (`OPENCODE_CONFIG` env var) - remplacements personnalisés
4. **Configuration du projet** (`igris.json` dans le projet) - paramètres spécifiques au projet
5. **Répertoires `.igris`** - agents, commandes, plugins
...
:::note
Les répertoires `.igris` et `~/.config/igris` utilisent des **noms au pluriel** pour les sous-répertoires : `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` et `themes/`. Les noms singuliers (par exemple, `agent/`) sont également pris en charge pour une compatibilité ascendante.
:::
...
Les organisations peuvent fournir une configuration par défaut via le point de terminaison `.well-known/igris`. Ceci est récupéré automatiquement lorsque vous vous authentifiez auprès d’un fournisseur qui le prend en charge.
...
Placez votre configuration globale OpenCode dans `~/.config/igris/igris.json`. Utilisez la configuration globale pour les préférences de l'utilisateur telles que les fournisseurs, les modèles et les autorisations.

Pour les paramètres spécifiques à TUI, utilisez `~/.config/igris/tui.json`.
...
Ajoutez `igris.json` à la racine de votre projet. La configuration du projet a la priorité la plus élevée parmi les fichiers de configuration standard : elle remplace les configurations globales et distantes.
...
Spécifiez un répertoire de configuration personnalisé à l'aide de `OPENCODE_CONFIG_DIR` variable d'environnement. Ce répertoire sera recherché pour les agents, les commandes, modes et plugins tout comme le répertoire standard `.igris`, et devrait suivre la même structure.
...
Le répertoire personnalisé est chargé après les répertoires de configuration globale et `.igris`, il **peut donc remplacer** leurs paramètres.

---

## Schéma

Le fichier de configuration a un schéma défini dans [**`igris.ai/config.json`**](https://igris.ai/config.json).

La configuration TUI utilise [**`igris.ai/tui.json`**](https://igris.ai/tui.json).
...
Les anciennes clés `theme`, `keybinds` et `tui` dans `igris.json` sont obsolètes et migrées automatiquement lorsque cela est possible.
...
Vous pouvez configurer les paramètres du serveur pour les commandes `igris serve` et `igris web` via l'option `server`.
...
- `mdnsDomain` - Nom de domaine personnalisé pour le service mDNS. La valeur par défaut est `igris.local`. Utile pour exécuter plusieurs instances sur le même réseau.
...
Vous pouvez également définir des agents à l'aide de fichiers markdown dans `~/.config/igris/agents/` ou `.igris/agents/`. [En savoir plus ici](/docs/agents).
...
Ce paramètre s'applique à toutes les interfaces : TUI, CLI (`igris run`), application de bureau et GitHub Action.
...
Vous pouvez également définir des commandes à l'aide de fichiers markdown dans `~/.config/igris/commands/` ou `.igris/commands/`. [En savoir plus ici](/docs/commands).
...
Par défaut, igris **autorise toutes les opérations** sans nécessiter d'approbation explicite. Vous pouvez modifier cela en utilisant l'option `permission`.
...
Placez les fichiers du plugin dans `.igris/plugins/` ou `~/.config/igris/plugins/`. Vous pouvez également charger des plugins depuis npm via l'option `plugin`.
