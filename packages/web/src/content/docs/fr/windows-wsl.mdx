...
1. **Exécutez `igris web` dans le terminal WSL** plutôt que dans PowerShell:
...
Lancer `igris web` depuis WSL garantit un accès correct au système de fichiers et une bonne intégration terminal, tout en restant accessible depuis votre navigateur Windows.
...
- Votre configuration OpenCode et vos sessions sont stockées dans l'environnement WSL à `~/.local/share/igris/`
