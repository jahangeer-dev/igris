...
- `.igris/plugins/` - Plugins au niveau du projet
- `~/.config/igris/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/igris/node_modules/`.
...
1. Configuration globale (`~/.config/igris/igris.json`)
2. Configuration du projet (`igris.json`)
3. Répertoire global des plugins (`~/.config/igris/plugins/`)
4. Répertoire des plugins du projet (`.igris/plugins/`)
...
- `client` : un client SDK igris pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre igris.
...
Empêchez igris de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à igris :
...
L'assistant `tool` crée un outil personnalisé que igris peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour igris aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://igris.ai/docs/sdk) pour plus de détails.
