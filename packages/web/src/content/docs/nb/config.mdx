---
title: Konfigurasjon
description: Ved å bruke igris JSON-konfigurasjonen.
---
...
For eksempel, hvis din globale konfigurasjon setter `theme: "igris"` og `autoupdate: true`, og prosjektkonfigurasjonen setter `model: "anthropic/claude-sonnet-4-5"`, vil den endelige konfigurasjonen inkludere alle tre innstillingene.
...
1. **Ekstern konfigurasjon** (fra `.well-known/igris`) - organisasjonsstandarder
2. **Global konfigurasjon** (`~/.config/igris/igris.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`igris.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.igris` kataloger** - agenter, kommandoer, plugins
...
:::note
`.igris` og `~/.config/igris` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Organisasjoner kan tilby standardkonfigurasjon via `.well-known/igris`-endepunktet. Dette hentes automatisk når du autentiserer med en leverandør som støtter det.
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/igris/igris.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `igris.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
moduser og plugins akkurat som standard `.igris` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.igris`-katalogene, slik at den **kan overstyre** innstillingene deres.

---

## Skjema

Konfigurasjonsfilen har et skjema som er definert i [**`igris.ai/config.json`**](https://igris.ai/config.json).
...
Du kan konfigurere serverinnstillinger for kommandoene `igris serve` og `igris web` gjennom alternativet `server`.
...
- `mdnsDomain` - Egendefinert domenenavn for mDNS-tjeneste. Standard er `igris.local`. Nyttig for å kjøre flere forekomster på samme nettverk.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/igris/agents/` eller `.igris/agents/`. [Les mer her](/docs/agents).
...
Denne innstillingen gjelder for alle grensesnitt: TUI, CLI (`igris run`), skrivebordsapp og GitHub Action.
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/igris/commands/` eller `.igris/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.igris/plugins/` eller `~/.config/igris/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
