...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\igris\log`

Loggfiler navngis med tidsstempler (f.eks. `2025-01-09T123456.log`) og de siste 10 loggfilene beholdes.

Du kan angi loggnivået med kommandolinjealternativet `--log-level` for å få mer detaljert feilsøkingsinformasjon. For eksempel `igris --log-level DEBUG`.
...
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop kjører en lokal OpenCode-server (`igris-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
- **macOS/Linux** (eldre installasjoner): `~/.local/share/igris/igris.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\igris\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> lim inn `~/.cache/igris`
- **Linux**: slett `~/.cache/igris` (eller kjør `rm -rf ~/.cache/igris`)
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.cache\igris`
...
Hvis `igris.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
...
- `igris.settings.dat` (stasjonær standardserver URL)
- `igris.global.dat` og `igris.workspace.*.dat` (UI tilstand som nylige servere/prosjekter)
...
1. Tøm leverandørens pakkebuffer:
   ...
   På Windows, trykk `WIN+R` og slett: `%USERPROFILE%\.cache\igris`
