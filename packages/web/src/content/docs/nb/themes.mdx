...
Som standard bruker OpenCode vårt eget `igris`-tema.
...
2. **Brukerkonfigurasjonskatalog** - Definert i `~/.config/igris/themes/*.json` eller `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.igris/themes/*.json`
4. **Gjeldende arbeidskatalog** - Definert i `./.igris/themes/*.json`
