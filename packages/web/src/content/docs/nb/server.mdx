...
Kommandoen `igris serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `igris.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `igris` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `igris serve` og `igris web`.
...
Når du kjører `igris` starter den en TUI og en server. Der TUI er
...
Du kan kjøre `igris serve` for å starte en frittstående server. Hvis du har
OpenCode TUI kjører, vil `igris serve` starte en ny server.
...
igris-serveren viser følgende APIer.
