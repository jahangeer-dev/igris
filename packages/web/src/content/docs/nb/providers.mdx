...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Zen` og gå til [igris.ai/auth](https://igris.ai/zen).
...
1. Kjør kommandoen `/connect` i TUI, velg `OpenCode Go`, og gå til [igris.ai/auth](https://igris.ai/zen).
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `igris.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i igris støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere igris til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
`igris.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `igris.json`:
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) plugin for automatisk å logge hver OpenCode-samtale som en økt i Helicone.
...
:::tip
Ollama kan automatisk konfigurere seg selv for OpenCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/igris) for detaljer.
:::
...
4. Opprett eller oppdater `igris.json`-filen i prosjektkatalogen:
...
1. **Sjekk autentiseringsoppsettet**: Kjør `igris auth list` for å se om legitimasjonen
