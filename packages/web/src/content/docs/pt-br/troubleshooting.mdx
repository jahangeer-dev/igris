...
Para depurar problemas com o igris, comece verificando os logs e os dados locais que ele armazena no disco.
...
- **macOS/Linux**: `~/.local/share/igris/log/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\igris\log`
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `igris --log-level DEBUG`.

---

## Armazenamento

O igris armazena dados de sessão e outros dados do aplicativo no disco em:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.local\share\igris`
...
O igris Desktop executa um servidor igris local (o sidecar `igris-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (ou `~/.config/igris/igris.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/igris/igris.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\igris\igris.jsonc`
...
O igris também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:

- **Plugins globais**
  - **macOS/Linux**: `~/.config/igris/plugins/`
  - **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\igris\plugins`
- **Plugins de projeto** (apenas se você usar configuração por projeto)
  - `<your-project>/.igris/plugins/`
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o igris possa reconstruí-lo.

1. Saia completamente do igris Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/igris`
- **Linux**: exclua `~/.cache/igris` (ou execute `rm -rf ~/.cache/igris`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\igris`

3. Reinicie o igris Desktop.

---

### Corrigindo problemas de conexão com o servidor

O igris Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `igris.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o igris Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do igris.

---

### Notificações não aparecendo

O igris Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o igris nas configurações do seu sistema operacional, e
...
1. Saia do igris Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo igris Desktop):

- `igris.settings.dat` (URL do servidor padrão do desktop)
- `igris.global.dat` e `igris.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o igris:
...
1. **Relatar problemas no GitHub**
   ...
   [**github.com/anomalyco/igris/issues**](https://github.com/anomalyco/igris/issues)
...
2. **Junte-se ao nosso Discord**
   ...
   [**igris.ai/discord**](https://igris.ai/discord)
...
3. Certifique-se de que você tem a versão mais recente com `igris upgrade`
...
Para descobrir quais modelos você tem acesso, execute `igris models`
...
2. Se o problema persistir, tente limpar sua configuração armazenada:
   ...
   ```bash
   rm -rf ~/.local/share/igris
   ```

   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.local\share\igris`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O igris instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
1. Limpe o cache do pacote de provedores:
   ...
   No Windows, pressione `WIN+R` e exclua: `%USERPROFILE%\.cache\igris`

2. Reinicie o igris para reinstalar os pacotes de provedores mais recentes

Isso forçará o igris a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O igris detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
