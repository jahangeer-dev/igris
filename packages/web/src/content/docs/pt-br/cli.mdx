---
title: CLI
description: Opções e comandos da CLI do igris.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do igris, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
igris
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o igris programaticamente.
...
Inicie a interface de usuário do terminal do igris.
...
A CLI do igris também possui os seguintes comandos.

---

### agent

Gerencie agentes para o igris.
...
Anexe um terminal a um servidor backend do igris já em execução, iniciado via comandos `serve` ou `web`.

```bash
igris attach [url]
```

Isso permite usar o TUI com um backend igris remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `igris`) |
...
O igris é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `igris auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/igris/auth.json`.

```bash
igris auth login
```

Quando o igris é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no igris.
...
Execute o igris em modo não interativo passando um prompt diretamente.
...
```bash "igris run"
igris run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `igris serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
igris run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor igris em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `igris`) |
...
Inicie um servidor igris sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
igris serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do igris sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `igris`).
...
Liste todas as sessões do igris.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do igris.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do igris.

```bash
igris import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do igris.
...
Inicie um servidor igris sem cabeça com uma interface web.

```bash
igris web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o igris através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `igris`).
...
Desinstale o igris e remova todos os arquivos relacionados.
...
Atualiza o igris para a versão mais recente ou uma versão específica.
...
A CLI do igris aceita as seguintes flags globais.
...
O igris pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `igris`) |
