...
Com o igris, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o igris usa nosso próprio tema `igris`.
...
O igris vem com vários temas embutidos.
...
- Querem que o igris corresponda à aparência do seu terminal
...
O igris suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/igris/themes/*.json` ou `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.igris/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.igris/themes/*.json`
