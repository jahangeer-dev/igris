---
title: Plugins
description: Escreva seus próprios plugins para estender o igris.
---

Plugins permitem que você estenda o igris conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do igris.
...
- `.igris/plugins/` - Plugins em nível de projeto
- `~/.config/igris/plugins/` - Plugins globais
...
```json title="igris.json"
{
  ...
  "plugin": ["igris-helicone-session", "igris-wakatime", "@my-org/custom-plugin"]
}
```
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/igris/node_modules/`.
...
1. Configuração global (`~/.config/igris/igris.json`)
2. Configuração do projeto (`igris.json`)
3. Diretório de plugins global (`~/.config/igris/plugins/`)
4. Diretório de plugins do projeto (`.igris/plugins/`)
...
O igris executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.

```ts title=".igris/plugins/my-plugin.ts"
...
```js title=".igris/plugins/example.js"
...
- `client`: Um cliente SDK do igris para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o igris.
...
```js title=".igris/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "igris"'`
      }
    },
  }
}
...
:::note
Se você estiver usando o aplicativo desktop igris, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o igris de ler arquivos `.env`:

```javascript title=".igris/plugins/env-protection.js"
...
```javascript title=".igris/plugins/inject-env.js"
...
Plugins também podem adicionar ferramentas personalizadas ao igris:

```ts title=".igris/plugins/custom-tools.ts"
import { type Plugin, tool } from "@igris-ai/plugin"
...
O helper `tool` cria uma ferramenta personalizada que o igris pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o igris junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://igris.ai/docs/sdk) para detalhes.
...
```ts title=".igris/plugins/compaction.ts"
import type { Plugin } from "@igris-ai/plugin"
...
```ts title=".igris/plugins/custom-compaction.ts"
import type { Plugin } from "@igris-ai/plugin"
