---
title: Servidor
description: Interaja com o servidor igris via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `igris serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente igris pode usar.
...
```bash
igris serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `igris.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
igris serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENCODE_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `igris`, ou defina `OPENCODE_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `igris serve` quanto ao `igris web`.

```bash
OPENCODE_SERVER_PASSWORD=your-password igris serve
```
...
Quando você executa `igris`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor igris para interagir com o igris programaticamente.
:::

Essa arquitetura permite que o igris suporte múltiplos clientes e permite que você interaja com o igris programaticamente.

Você pode executar `igris serve` para iniciar um servidor autônomo. Se você tiver o TUI do igris em execução, `igris serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do igris [IDE](/docs/ide).
...
O servidor igris expõe as seguintes APIs.
