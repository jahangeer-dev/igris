...
As habilidades do agente permitem que o igris descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O igris pesquisa nesses locais:

- Configuração do projeto: `.igris/skills/<name>/SKILL.md`
- Configuração global: `~/.config/igris/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o igris sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.igris/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/igris/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.igris/skills/git-release/SKILL.md` assim:
...
O igris lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `igris.json`:
...
**Para agentes embutidos** (em `igris.json`):
