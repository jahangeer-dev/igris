...
As ferramentas permitem que o LLM execute ações em sua base de código. O igris vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no igris.
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor igris ou quando a variável de ambiente `OPENCODE_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o igris:

```bash
OPENCODE_ENABLE_EXA=1 igris
```
