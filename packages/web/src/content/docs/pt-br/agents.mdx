...
Existem dois tipos de agentes no igris; agentes primários e subagentes.
...
igris vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Configure os agentes em seu arquivo de configuração `igris.json`:
...
- Global: `~/.config/igris/agents/`
- Por projeto: `.igris/agents/`

```markdown title="~/.config/igris/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o igris usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do igris quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do igris usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenCode Zen](/docs/zen), você usaria `igris/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="igris.json" {3-6,9-12}
{
  "$schema": "https://igris.ai/config.json",
  "tools": {
    "write": true,
...
```json title="igris.json" {3-5,8-10}
{
  "$schema": "https://igris.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `igris models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/igris).
:::
...
```markdown title="~/.config/igris/agents/docs-writer.md"
---
...
```markdown title="~/.config/igris/agents/security-auditor.md"
---
