...
1. **Esegui `igris web` nel terminale WSL** invece che in PowerShell:
...
Eseguire `igris web` da WSL garantisce accesso corretto al file system e integrazione del terminale, restando comunque accessibile dal browser Windows.
...
- Configurazione e sessioni di OpenCode sono salvate nell'ambiente WSL in `~/.local/share/igris/`
