...
:::tip[Utenti Windows]
Per la migliore esperienza, esegui `igris web` da [WSL](/docs/windows-wsl) invece che da PowerShell. Questo garantisce accesso corretto al file system e integrazione col terminale.
:::
...
Questo imposta automaticamente l'hostname a `0.0.0.0` e pubblicizza il server come `igris.local`.
...
Lo username predefinito e `igris`, ma puo essere cambiato con `OPENCODE_SERVER_USERNAME`.
...
Puoi anche configurare le impostazioni del server nel file di config `igris.json`:
