...
Configura gli agenti nel file `igris.json`:
...
- Globale: `~/.config/igris/agents/`
- Per progetto: `.igris/agents/`
...
L'ID modello nella configurazione OpenCode usa il formato `provider/model-id`. Per esempio, se stai usando [OpenCode Zen](/docs/zen), useresti `igris/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `igris models` per vedere la lista dei modelli disponibili.
:::
...
:::tip
Hai un agente che vorresti condividere? [Invia una PR](https://github.com/anomalyco/igris).
:::
