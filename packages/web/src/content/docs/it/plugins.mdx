...
- `.igris/plugins/` - plugin a livello progetto
- `~/.config/igris/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/igris/node_modules/`.
...
1. Config globale (`~/.config/igris/igris.json`)
2. Config di progetto (`igris.json`)
3. Directory plugin globale (`~/.config/igris/plugins/`)
4. Directory plugin di progetto (`.igris/plugins/`)
...
- `client`: un client SDK di igris per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere igris.
...
Impedisci a igris di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a igris:
...
L'helper `tool` crea uno strumento personalizzato che igris può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in igris insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://igris.ai/docs/sdk) per i dettagli.
