...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\igris\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `igris --log-level DEBUG`.

---

## Archiviazione

igris salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop esegue in background un server locale di OpenCode (il sidecar `igris-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/igris/igris.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\igris\plugins`
...
- **Linux**: elimina `~/.cache/igris` (oppure esegui `rm -rf ~/.cache/igris`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\igris`
...
Se il tuo `igris.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
- `igris.settings.dat` (desktop default server URL)
- `igris.global.dat` e `igris.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
3. Assicurati di avere l'ultima versione con `igris upgrade`
...
Per capire a quali modelli hai accesso, esegui `igris models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\igris`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. igris installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\igris`

2. Riavvia igris per reinstallare i pacchetti provider piu recenti

Questo forzera igris a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
igris rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
