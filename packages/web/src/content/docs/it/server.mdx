---
title: Server
description: Interagisci con il server igris via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `igris serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client igris.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `igris.local` |
...
Imposta `OPENCODE_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `igris`, oppure imposta `OPENCODE_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `igris serve` sia per `igris web`.
...
Quando esegui `igris` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server igris per interagire con igris in modo programmatico.
:::

Questa architettura permette a igris di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `igris serve` per avviare un server standalone. Se la TUI di igris e gia in esecuzione, `igris serve` avviera un nuovo server.
...
Il server igris espone le seguenti API.
