...
1. **Config remota** (da `.well-known/igris`) - default dell'organizzazione
2. **Config globale** (`~/.config/igris/igris.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`igris.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.igris`** - agenti, comandi, plugin
...
:::note
Le directory `.igris` e `~/.config/igris` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/igris`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di OpenCode in `~/.config/igris/igris.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/igris/tui.json`.
...
Aggiungi `igris.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.igris` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.igris`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`igris.ai/config.json`**](https://igris.ai/config.json).

La configurazione TUI usa [**`igris.ai/tui.json`**](https://igris.ai/tui.json).
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `igris.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `igris serve` e `igris web` tramite l'opzione `server`.
...
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `igris.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi anche definire agenti usando file markdown in `~/.config/igris/agents/` o `.igris/agents/`. [Scopri di piu](/docs/agents).
...
Questa impostazione si applica a tutte le interfacce: TUI, CLI (`igris run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/igris/commands/` o `.igris/commands/`. [Scopri di piu](/docs/commands).
...
Per impostazione predefinita, igris **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
Metti i file dei plugin in `.igris/plugins/` o `~/.config/igris/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
