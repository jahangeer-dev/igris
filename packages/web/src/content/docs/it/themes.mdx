...
Per impostazione predefinita, OpenCode usa il tema `igris`.
...
2. **Directory di configurazione utente** - in `~/.config/igris/themes/*.json` o `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.igris/themes/*.json`
4. **Directory di lavoro corrente** - in `./.igris/themes/*.json`
