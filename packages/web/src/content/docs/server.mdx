---
title: Server
description: Interact with igris server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `igris serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an igris client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `igris.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `igris`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `igris serve` and `igris web`.
...
When you run `igris` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the igris server to interact with igris programmatically.
:::

This architecture lets igris support multiple clients and allows you to interact with igris programmatically.

You can run `igris serve` to start a standalone server. If you have the
igris TUI running, `igris serve` will start a new server.
...
The igris server exposes the following APIs.
