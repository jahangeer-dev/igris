...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\igris\log` を貼り付けます

ログファイルにはタイムスタンプ付きの名前が付けられ (例: `2025-01-09T123456.log`)、最新の 10 個のログファイルが保存されます。

`--log-level` コマンドラインオプションを使用してログレベルを設定すると、より詳細なデバッグ情報を取得できます。たとえば、`igris --log-level DEBUG`。
...
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.local\share\igris` を貼り付けます
...
OpenCode Desktop は、ローカル OpenCode サーバー (`igris-cli` サイドカー) をバックグラウンドで実行します。ほとんどの問題は、誤動作しているプラグイン、破損したキャッシュ、または不正なサーバー設定によって発生します。
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (または `~/.config/igris/igris.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/igris/igris.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\igris\igris.jsonc` を貼り付けます
...
- **グローバルプラグイン**
  ...
  - **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\igris\plugins` を貼り付けます
...
- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/igris` を貼り付け
- **Linux**: `~/.cache/igris` を削除します (または `rm -rf ~/.cache/igris` を実行します)。
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.cache\igris` を貼り付けます
...
`igris.json(c)` に `server` セクションが含まれている場合は、それを一時的に削除し、デスクトップアプリを再起動します。
...
- `igris.settings.dat` (デスクトップのデフォルトサーバー URL)
- `igris.global.dat` および `igris.workspace.*.dat` (最近のサーバー/プロジェクトなどの UI 状態)
...
3. `igris upgrade` を含む最新バージョンを使用していることを確認してください
...
どのモデルにアクセスできるかを確認するには、`igris models` を実行します。
...
2. 問題が解決しない場合は、保存されている構成をクリアしてみてください。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.local\share\igris` を削除します。
...
1. プロバイダーパッケージのキャッシュをクリアします。
   ...
   Windows では、`WIN+R` を押して、`%USERPROFILE%\.cache\igris` を削除します。
