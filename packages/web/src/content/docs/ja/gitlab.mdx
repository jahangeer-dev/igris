...
ここでは、コミュニティが作成した OpenCode 用の CI/CD コンポーネント — [nagyv/gitlab-igris](https://gitlab.com/nagyv/gitlab-igris).
...
さらに多くの入力と使用例については、このコンポーネントの [ドキュメント](https://gitlab.com/explore/catalog/nagyv/gitlab-igris) をチェックしてください。
...
コメントで `@igris` に言及すると、OpenCode が GitLab CI パイプライン内でタスクを実行します。
...
:::tip
`@igris` とは異なるトリガーフレーズを使用するように設定できます。
:::
