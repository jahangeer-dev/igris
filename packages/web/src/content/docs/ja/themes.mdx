...
デフォルトでは、OpenCode は独自の `igris` テーマを使用します。
...
2. **ユーザー設定ディレクトリ** - `~/.config/igris/themes/*.json` または `$XDG_CONFIG_HOME/igris/themes/*.json` で定義されます
3. **プロジェクトのルートディレクトリ** - `<project-root>/.igris/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.igris/themes/*.json` で定義
...
1. **組み込みテーマ** - これらはバイナリに埋め込まれています
2. **ユーザー設定ディレクトリ** - `~/.config/igris/themes/*.json` または `$XDG_CONFIG_HOME/igris/themes/*.json` で定義されます
3. **プロジェクトのルート ディレクトリ** - `<project-root>/.igris/themes/*.json` で定義されます。
4. **現在の作業ディレクトリ** - `./.igris/themes/*.json` で定義
