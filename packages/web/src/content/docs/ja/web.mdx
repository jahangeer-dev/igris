...
:::
:::tip[Windows ユーザー]
最高のエクスペリエンスを得るには、PowerShell ではなく [WSL](/docs/windows-wsl) から `igris web` を実行します。これにより、ファイルシステムへの適切なアクセスとターミナルの統合が保証されます。
:::
...
これにより、ホスト名が自動的に `0.0.0.0` に設定され、サーバーが `igris.local` としてアドバタイズされます。
...
ユーザー名のデフォルトは `igris` ですが、`OPENCODE_SERVER_USERNAME` で変更できます。
...
`igris.json` 設定ファイルでサーバー設定を構成することもできます。
