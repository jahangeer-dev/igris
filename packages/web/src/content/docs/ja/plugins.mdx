...
- `.igris/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/igris/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/igris/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/igris/igris.json`)
2. プロジェクト設定 (`igris.json`)
3. グローバルプラグインディレクトリ (`~/.config/igris/plugins/`)
4. プロジェクトプラグインディレクトリ (`.igris/plugins/`)
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://igris.ai/docs/sdk) を参照してください。
