...
1. **PowerShell ではなく WSL ターミナルで `igris web` を実行する**
...
WSL から `igris web` を実行すると、適切なファイルシステムアクセスとターミナル統合を維持したまま、Windows ブラウザから利用できます。
...
- OpenCode の設定とセッションは WSL 環境内の `~/.local/share/igris/` に保存されます
