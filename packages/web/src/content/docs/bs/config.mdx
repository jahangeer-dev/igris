...
1. **Udaljena konfiguracija** (od `.well-known/igris`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/igris/igris.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`igris.json` u projektu) - postavke specifične za projekat
5. **`.igris` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.igris` i `~/.config/igris` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/igris` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/igris/igris.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/igris/tui.json`.
...
Dodajte `igris.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.igris` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.igris`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`igris.ai/config.json`**](https://igris.ai/config.json).

TUI konfiguracija koristi [**`igris.ai/tui.json`**](https://igris.ai/tui.json).
...
Stari `theme`, `keybinds`, i `tui` ključevi u `igris.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `igris serve` i `igris web` putem opcije `server`.
...
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `igris.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/igris/agents/` ili `.igris/agents/`. [Saznajte više ovdje](/docs/agents).
...
Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`igris run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/igris/commands/` ili `.igris/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.igris/plugins/` ili `~/.config/igris/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
