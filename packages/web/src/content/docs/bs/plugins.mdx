...
- `.igris/plugins/` - Dodaci na nivou projekta
- `~/.config/igris/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/igris/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/igris/igris.json`)
2. Konfiguracija projekta (`igris.json`)
3. Globalni direktorij dodataka (`~/.config/igris/plugins/`)
4. Direktorij dodataka projekta (`.igris/plugins/`)
...
Spriječite igris da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u igris:
...
Pomoćnik `tool` kreira prilagođeni alat koji igris može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za igris zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://igris.ai/docs/sdk) za detalje.
