...
Za najbolje iskustvo, pokrenite `igris web` iz [WSL](/docs/windows-wsl) umjesto PowerShell-a. Ovo osigurava pravilan pristup sistemu datoteka i integraciju terminala.
:::
...
Ovo automatski postavlja ime hosta na `0.0.0.0` i oglašava server kao `igris.local`.
...
Korisničko ime podrazumevano je `igris`, ali se može promeniti sa `OPENCODE_SERVER_USERNAME`.
...
Također možete konfigurirati postavke servera u svom `igris.json` konfiguracijskom fajlu:
