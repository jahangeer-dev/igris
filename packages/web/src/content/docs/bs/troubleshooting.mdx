...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\igris\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `igris --log-level DEBUG`.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop pokreće lokalni OpenCode server (`igris-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (ili `~/.config/igris/igris.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/igris/igris.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Globalni dodaci**
  ...
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\igris\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/igris`
- **Linux**: obrišite `~/.cache/igris` (ili pokrenite `rm -rf ~/.cache/igris`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\igris`
...
Ako vaš `igris.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
- `igris.settings.dat` (URL zadanog servera za desktop)
- `igris.global.dat` i `igris.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `igris upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `igris models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\igris`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\igris`
