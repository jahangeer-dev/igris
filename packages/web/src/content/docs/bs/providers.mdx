...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite igris i idite na [igris.ai/auth](https://igris.ai/auth).
...
1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenCode Go` i idite na [igris.ai/auth](https://igris.ai/zen).
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete igris:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `igris.json`:
...
Možete konfigurirati igris za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi igris ispravno radio.
   :::
...
Također možete dodati modele kroz svoju igris konfiguraciju.
...
da pohrani token u igris auth memoriju.
:::
...
`igris.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth).

##### Konfiguracija

Prilagodite putem `igris.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s igris:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće igris.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem igrisa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) da automatski prijavite svaki OpenCode razgovor kao sesiju u Helicone-u.
...
Možete konfigurirati igris za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati igris za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati igris za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/igris) za detalje.
:::
...
Također možete dodati dodatne modele putem vaše igris konfiguracije.
...
5. Također ih možete prilagoditi putem vaše igris konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju igris konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s igris-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `igris.json` datoteku u direktoriju projekta:
...
1. **Provjerite postavke autentifikacije**: Pokrenite `igris auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj igris konfiguraciji.
