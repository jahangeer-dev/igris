...
Po defaultu, OpenCode koristi nasu `igris` temu.
...
2. **Korisnicki config direktorij** - `~/.config/igris/themes/*.json` ili `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.igris/themes/*.json`
4. **Trenutni radni direktorij** - `./.igris/themes/*.json`
