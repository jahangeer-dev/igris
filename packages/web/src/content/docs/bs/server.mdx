---
title: Server
description: Komunicirajte s igris serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `igris serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji igris klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `igris.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `igris`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `igris serve` i za `igris web`.
...
Kada pokrenete `igris`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite igris server za programsku interakciju sa igris-om.
:::

Ova arhitektura omogucava igris podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `igris serve` da startate standalone server. Ako je igris TUI vec pokrenut, `igris serve` ce pokrenuti novi server.
...
igris server izlaže sljedece API-je.
