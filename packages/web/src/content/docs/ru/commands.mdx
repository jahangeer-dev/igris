...
Создайте `.igris/commands/test.md`:

```md title=".igris/commands/test.md"
---
...
Вы можете добавлять собственные команды через конфигурацию igris или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем igris [config](/docs/config):

```json title="igris.jsonc" {4-12}
{
...
- Глобальный: `~/.config/igris/commands/`
- Для каждого проекта: `.igris/commands/`

```markdown title="~/.config/igris/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".igris/commands/component.md"
---
...
```md title=".igris/commands/create-file.md"
---
...
```md title=".igris/commands/analyze-coverage.md"
---
...
```md title=".igris/commands/review-changes.md"
---
...
```md title=".igris/commands/review-component.md"
---
...
igris включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
