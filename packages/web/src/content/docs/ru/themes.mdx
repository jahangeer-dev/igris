...
С помощью igris вы можете выбрать одну из нескольких встроенных тем, использовать тему, которая адаптируется к теме вашего терминала, или определить свою собственную тему.

По умолчанию igris использует нашу собственную тему `igris`.
...
igris поставляется с несколькими встроенными темами.
...
- Хотите, чтобы igris соответствовал внешнему виду их терминала
...
igris поддерживает гибкую систему тем на основе JSON, которая позволяет пользователям легко создавать и настраивать темы.
...
2. **Каталог конфигурации пользователя** – определяется в `~/.config/igris/themes/*.json` или `$XDG_CONFIG_HOME/igris/themes/*.json`.
3. **Корневой каталог проекта** – определено в `<project-root>/.igris/themes/*.json`.
4. **Текущий рабочий каталог** – определено в `./.igris/themes/*.json`.
...
```bash no-frame
mkdir -p ~/.config/igris/themes
vim ~/.config/igris/themes/my-theme.json
```

Для тем проекта:

```bash no-frame
mkdir -p .igris/themes
vim .igris/themes/my-theme.json
```
...
{
  "$schema": "https://igris.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
