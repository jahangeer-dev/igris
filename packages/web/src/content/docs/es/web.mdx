...
Para obtener la mejor experiencia, ejecute `igris web` desde [WSL](/docs/windows-wsl) en lugar de PowerShell. Esto garantiza el acceso adecuado al sistema de archivos y la integración del terminal.
:::
...
Esto establece automáticamente el nombre de host en `0.0.0.0` y anuncia el servidor como `igris.local`.
...
El nombre de usuario predeterminado es `igris` pero se puede cambiar con `OPENCODE_SERVER_USERNAME`.
...
También puede configurar los ajustes del servidor en su archivo de configuración `igris.json`:
