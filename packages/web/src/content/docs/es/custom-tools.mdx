...
description: Cree herramientas que LLM pueda llamar en igris.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de igris como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.igris/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/igris/tools/`.
...
```ts title=".igris/tools/database.ts" {1}
...
```ts title=".igris/tools/project.ts" {8}
import { tool } from "@igris-ai/plugin"
...
```python title=".igris/tools/add.py"
import sys
...
```ts title=".igris/tools/python-add.ts" {10}
import { tool } from "@igris-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".igris/tools/add.py")
