...
- `.igris/plugins/` - Complementos a nivel de proyecto
- `~/.config/igris/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/igris/node_modules/`.
...
1. Configuración global (`~/.config/igris/igris.json`)
2. Configuración del proyecto (`igris.json`)
3. Directorio global de complementos (`~/.config/igris/plugins/`)
4. Directorio de complementos del proyecto (`.igris/plugins/`)
...
- `client`: Un cliente SDK igris para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar igris.
...
Evite que igris lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a igris:
...
El ayudante `tool` crea una herramienta personalizada a la que igris puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para igris junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://igris.ai/docs/sdk) para obtener más detalles.
