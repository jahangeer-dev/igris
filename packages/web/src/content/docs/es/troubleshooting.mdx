...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\igris\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `igris --log-level DEBUG`.

---

## Almacenamiento

igris almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop ejecuta un servidor OpenCode local (el sidecar `igris-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (o `~/.config/igris/igris.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/igris/igris.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Complementos globales**
  ...
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\igris\plugins`
...
- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/igris`
- **Linux**: eliminar `~/.cache/igris` (o ejecutar `rm -rf ~/.cache/igris`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\igris`
...
Si su `igris.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
- `igris.settings.dat` (URL del servidor predeterminado de escritorio)
- `igris.global.dat` y `igris.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `igris upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `igris models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\igris`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. igris instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\igris`

2. Reinicie igris para reinstalar los paquetes del proveedor más recientes.

Esto obligará a igris a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
igris detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
