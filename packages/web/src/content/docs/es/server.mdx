---
title: Servidor
description: Interactuar con el servidor igris a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `igris serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente igris puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `igris.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `igris`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `igris serve` como a `igris web`.
...
Cuando ejecuta `igris`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor igris para interactuar con igris mediante programación.
:::

Esta arquitectura permite que igris admita múltiples clientes y le permite interactuar con igris mediante programación.

Puede ejecutar `igris serve` para iniciar un servidor independiente. Si tienes el
igris TUI ejecutándose, `igris serve` iniciará un nuevo servidor.
...
El servidor igris expone las siguientes API.
