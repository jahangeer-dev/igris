...
1. Ejecute el comando `/connect` en TUI, seleccione igris y diríjase a [igris.ai/auth](https://igris.ai/auth).
...
1. Ejecute el comando `/connect` en TUI, seleccione `OpenCode Go` y diríjase a [igris.ai/auth](https://igris.ai/zen).
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta igris:
   ...
   Para una configuración persistente o específica del proyecto, utilice `igris.json`:
...
Puedes configurar igris para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que igris funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración igris.
...
para almacenar el token en el almacenamiento de autenticación igris.
:::
...
Archivo `igris.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth).

##### Configuración

Personalizar a través de `igris.json`:
...
Para utilizar su suscripción GitHub Copilot con igris:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta igris.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de igris, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) para registrar automáticamente cada conversación OpenCode como una sesión en Helicone.
...
Puede configurar igris para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar igris para usar modelos locales a través de LM Studio.
...
Puedes configurar igris para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para OpenCode. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/igris) para obtener más detalles.
:::
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración igris.
...
5. También puedes personalizarlos a través de tu configuración igris. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
También puede personalizar modelos a través de su configuración igris. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con igris. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `igris.json` en el directorio de su proyecto:
...
1. **Verifique la configuración de autenticación**: Ejecute `igris auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración igris y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración igris.
