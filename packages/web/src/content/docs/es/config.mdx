...
Por ejemplo, si su configuración global establece `theme: "igris"` y `autoupdate: true`, y la configuración de su proyecto establece `model: "anthropic/claude-sonnet-4-5"`, la configuración final incluirá las tres configuraciones.
...
1. **Configuración remota** (de `.well-known/igris`): valores predeterminados de la organización
2. **Configuración global** (`~/.config/igris/igris.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`igris.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.igris`** - agentes, comandos, complementos
...
:::note
Los directorios `.igris` y `~/.config/igris` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Las organizaciones pueden proporcionar una configuración predeterminada a través del punto final `.well-known/igris`. Esto se obtiene automáticamente cuando se autentica con un proveedor que lo admita.
...
Coloque su configuración global OpenCode en `~/.config/igris/igris.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `igris.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
modos y complementos como el directorio estándar `.igris`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.igris`, por lo que **puede anular** su configuración.

---

## Esquema

El archivo de configuración tiene un esquema definido en [**`igris.ai/config.json`**](https://igris.ai/config.json).
...
Puede configurar los ajustes del servidor para los comandos `igris serve` y `igris web` a través de la opción `server`.
...
- `mdnsDomain` - Nombre de dominio personalizado para el servicio mDNS. El valor predeterminado es `igris.local`. Útil para ejecutar múltiples instancias en la misma red.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/igris/agents/` o `.igris/agents/`. [Más información aquí](/docs/agents).
...
Esta configuración se aplica en todas las interfaces: TUI, CLI (`igris run`), aplicación de escritorio y GitHub Acción.
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/igris/commands/` o `.igris/commands/`. [Más información aquí](/docs/commands).
...
De forma predeterminada, igris **permite todas las operaciones** sin requerir aprobación explícita. Puede cambiar esto usando la opción `permission`.
...
Coloque los archivos de complemento en `.igris/plugins/` o `~/.config/igris/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
