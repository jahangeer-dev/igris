...
**npm 插件**在启动时使用 Bun 自动安装。包及其依赖项会缓存在 `~/.cache/igris/node_modules/` 中。
...
1. 全局配置 (`~/.config/igris/igris.json`)
2. 项目配置 (`igris.json`)
3. 全局插件目录 (`~/.config/igris/plugins/`)
4. 项目插件目录 (`.igris/plugins/`)
...
日志级别：`debug`、`info`、`warn`、`error`。详情请参阅 [SDK 文档](https://igris.ai/docs/sdk)。
