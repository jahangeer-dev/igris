...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\igris\log`

日志文件以时间戳命名（例如 `2025-01-09T123456.log`），并保留最近的 10 个日志文件。

你可以通过 `--log-level` 命令行选项设置日志级别以获取更详细的调试信息。例如：`igris --log-level DEBUG`。
...
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop 会在后台运行一个本地 OpenCode 服务器（即 `igris-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。
...
- **macOS/Linux**（旧版安装）: `~/.local/share/igris/igris.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **全局插件**
  ...
  - **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\igris\plugins`
- **项目插件**（仅当你使用了项目级配置时）
...
- **macOS**: Finder -> `Cmd+Shift+G` -> 粘贴 `~/.cache/igris`
- **Linux**: 删除 `~/.cache/igris`（或运行 `rm -rf ~/.cache/igris`）
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.cache\igris`
...
如果你的 `igris.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。
...
- `igris.settings.dat`（桌面默认服务器 URL）
- `igris.global.dat` 和 `igris.workspace.*.dat`（UI 状态，如最近的服务器/项目）
...
3. 使用 `igris upgrade` 确保你使用的是最新版本
...
2. 如果问题仍然存在，请尝试清除已存储的配置：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.local\share\igris`
...
1. 清除提供商包缓存：
   ...
   在 Windows 上，按 `WIN+R` 并删除：`%USERPROFILE%\.cache\igris`
