...
:::tip[Windows 用户]
为获得最佳体验，建议从 [WSL](/docs/windows-wsl) 而非 PowerShell 运行 `igris web`。这可以确保正确的文件系统访问和终端集成。
:::
...
这会自动将主机名设置为 `0.0.0.0`，并将服务器广播为 `igris.local`。
...
用户名默认为 `igris`，可以通过 `OPENCODE_SERVER_USERNAME` 进行更改。
