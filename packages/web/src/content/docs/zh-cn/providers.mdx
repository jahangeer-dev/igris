...
1. 在 TUI 中执行 `/connect` 命令，选择 igris，然后前往 [igris.ai/auth](https://igris.ai/auth)。
...
你可以通过 [Atomic Chat](https://atomic.chat) 配置 igris 以使用本地模型。Atomic Chat 是一款桌面应用程序，它在 OpenAI 兼容的 API 服务器后面运行本地 LLM（默认端点 `http://127.0.0.1:1337/v1`）。
...
:::note[合规说明]
OpenCode 会使用一个小模型来执行部分 AI 任务，例如生成会话标题。默认情况下使用由 Zen 托管的 gpt-5-nano。如果你需要让 OpenCode 仅使用你自己的 GitLab 托管实例，请在 `igris.json` 文件中添加以下内容。同时建议禁用会话共享。
...
更多文档请参阅 [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth) 主页。

##### 配置

通过 `igris.json` 进行自定义配置：
...
Helicone 的 [Sessions](https://docs.helicone.ai/features/sessions) 功能允许你将相关的 LLM 请求归为一组。使用 [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) 插件可以自动将每个 OpenCode 对话记录为 Helicone 中的一个会话。
...
:::tip
Ollama 可以自动为 OpenCode 进行配置。详见 [Ollama 集成文档](https://docs.ollama.com/integrations/igris)。
:::
...
1. **检查认证设置**：运行 `igris auth list` 查看该提供商的凭据是否已添加到配置中。
