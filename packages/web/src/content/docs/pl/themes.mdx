...
Dzięki igris możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie igris używa naszego własnego motywu `igris`.
...
igris ma kilka wbudowanych motywów.
...
igris obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/igris/themes/*.json` lub `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.igris/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.igris/themes/*.json`
