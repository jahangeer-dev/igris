...
Możesz dodać dodatkowe informacje do igris za pomocą _Model Context Protocol_, w skrócie MCP. igris obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [igris Config](https://igris.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/igris`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
igris automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, igris:
...
Jeśli serwer wymaga uwierzytelnienia, igris poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `igris mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji igris bezpieczne przechowa tokeny w `~/.local/share/igris/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w igris, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie igris, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć igris z kontem Sentry.
