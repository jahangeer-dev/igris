...
Aby debugować problemy z igris, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\igris\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `igris --log-level DEBUG`.

---

## Przechowywanie danych

igris przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\igris`
...
igris Desktop uruchamia w tle lokalny serwer igris (sidecar `igris-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (lub `~/.config/igris/igris.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/igris/igris.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\igris\igris.jsonc`
...
igris może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/igris/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\igris\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby igris mógł ją odbudować.

1. Całkowicie zamknij igris Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/igris`
- **Linux**: usuń `~/.cache/igris` (lub uruchom `rm -rf ~/.cache/igris`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\igris`

3. Uruchom ponownie igris Desktop.

---

### Napraw problemy z połączeniem z serwerem

igris Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `igris.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows igris Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami igris.

---

### Brak powiadomień

igris Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla igris w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie igris Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji igris Desktop):

- `igris.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `igris.global.dat` i `igris.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `igris upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `igris models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\igris`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. igris dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\igris`

2. Uruchom ponownie kod igris, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to igris do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
igris wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
