---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w igris.
---

import config from "../../../../config.mjs"
export const console = config.console

igris używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji igris.
...
w `~/.local/share/igris/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim igris
config.
...
OpenCode Zen to lista modeli dostarczonych przez zespół igris, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z igris. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Zen` i przejdź do [igris.ai/auth](https://igris.ai/zen).
...
Działa jak każdy inny dostawca w igris i jest całkowicie opcjonalny w użyciu.

---

## OpenCode Go

OpenCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół igris, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z igris.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenCode Go` i przejdź do [igris.ai/auth](https://igris.ai/zen).
...
Aby używać Amazon Bedrock z igris:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania igris:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `igris.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w igris nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować igris do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod igris działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji igris.
...
igris używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować igris
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`igris.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth).

##### Konfiguracja

Customize through `igris.json`:
...
Aby korzystać z subskrypcji GitHub Copilot z kodem igris:
...
Aby używać Google Vertex AI z igris:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania igris.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą igris, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session), aby automatycznie rejestrować każdą konwersację igris jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja igris będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować igris tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować igris tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować igris tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem igris. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/igris).
:::
...
Aby korzystać z Ollama Cloud z igris:
...
3. Skopiuj klucz API do użycia w igris.
...
6. **Ważne**: Przed użyciem modeli chmurowych w igris musisz pobrać informacje o modelu lokalnie:
...
OpenCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół igris. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji igris.
...
5. Możesz je także dostosować za pomocą konfiguracji igris. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z igris:
...
Możesz także dostosować modele za pomocą konfiguracji igris. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z igris. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `igris.json` w katalogu projektu:
...
Pola `limit` pozwalają igris zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `igris auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację igris i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji igris.
