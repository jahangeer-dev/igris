...
Umiejętności agenta pozwalają igris odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
igris przeszukuje te lokalizacje:

- Project config: `.igris/skills/<name>/SKILL.md`
- Global config: `~/.config/igris/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu igris przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.igris/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/igris/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.igris/skills/git-release/SKILL.md` w ten sposób:
...
igris wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `igris.json`:
...
**Dla agentów wbudowanych** (w `igris.json`):
