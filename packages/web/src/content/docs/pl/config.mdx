...
Na przykład, jeśli konfiguracja globalna ustawi `theme: "igris"` i `autoupdate: true`, a konfiguracja projektu ustawi `model: "anthropic/claude-sonnet-4-5"`, ostateczna konfiguracja będzie zawierać wszystkie trzy ustawienia.
...
1. **Konfiguracja zdalna** (z `.well-known/igris`) – ustawienia wymuszane przez organizację
2. **Konfiguracja globalna** (`~/.config/igris/igris.json`) – preferencje użytkownika
3. **Konfiguracja niestandardowa** (zmienna `OPENCODE_CONFIG`) — jawne nadpisanie
4. **Konfiguracja projektu** (`igris.json` w projekcie) - ustawienia specyficzne dla projektu
5. **Katalogi `.igris`** - agenci, polecenia, umiejętności
...
:::note
W katalogach `.igris` i `~/.config/igris` używane są **nazwy w liczbie mnogiej** dla podkatalogów: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Nazwy w liczbie pojedynczej (np. `agent/`) są również obsługiwane dla kompatybilności wstecznej.
:::
...
Organizacje mogą publikować konfigurację pod adresem `.well-known/igris`. Jest ona pobierana automatycznie po uwierzytelnieniu u dostawcy, który ją obsługuje.
...
Umieść swoją globalną konfigurację OpenCode w `~/.config/igris/igris.json`. Użyj jej do ustawień ogólnych dla użytkownika, takich jak dostawcy, modele i uprawnienia.

Dla ustawień specyficznych dla TUI, użyj `~/.config/igris/tui.json`.
...
Dodaj `igris.json` w katalogu głównym projektu. Konfiguracja projektu ma najwyższy priorytet wśród plików konfiguracyjnych — nadpisuje konfiguracje globalne i zdalne.
...
Możesz ustawić niestandardowy katalog konfiguracyjny za pomocą zmiennej środowiskowej `OPENCODE_CONFIG_DIR`. Katalog ten będzie przeszukiwany pod kątem agentów, poleceń, trybów i wtyczek, tak jak standardowy katalog `.igris` i powinien zachować tę samą strukturę.
...
Katalog niestandardowy jest ładowany po globalnej konfiguracji i katalogach `.igris`, więc **może być łączony** z ich zawartością.

---

## Schemat

Plik konfiguracyjny ma schemat JSON dostępny pod adresem [**`igris.ai/config.json`**](https://igris.ai/config.json).
...
Przestarzałe klucze `theme`, `keybinds` i `tui` w `igris.json` są wycofywane i automatycznie migrowane, gdy to możliwe.
...
Możesz skonfigurować serwer dla `igris serve` i `igris web` za pomocą opcji `server`.
...
- `mdnsDomain` - Niestandardowa nazwa domeny dla usług mDNS. Domyślnie `igris.local`. Przydatne do uruchamiania wielu instancji w tej samej sieci.
...
Możesz także definiować agentów przy użyciu plików markdown w `~/.config/igris/agents/` lub `.igris/agents/`. [Dowiedz się więcej o agentach](/docs/agents).
...
Dotyczy to wszystkich interfejsów: TUI, CLI (`igris run`), aplikacji desktopowej i GitHub Actions.
...
Możesz także definiować polecenia przy użyciu plików Markdown w `~/.config/igris/commands/` lub `.igris/commands/`. [Dowiedz się więcej o poleceniach](/docs/commands).
...
Umieść pliki wtyczek w `.igris/plugins/` lub `~/.config/igris/plugins/`. Możesz także załadować wtyczkę z npm za pomocą opcji `plugin`.
