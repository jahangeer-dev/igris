---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć igris.
---

Wtyczki umożliwiające wyłączenie igris poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania igris.
...
- `.igris/plugins/` - Wtyczki na poziomie projektu
- `~/.config/igris/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/igris/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/igris/igris.json`)
2. Konfiguracja projektu (`igris.json`)
3. Globalny katalog wtyczek (`~/.config/igris/plugins/`)
4. Katalog wtyczek projektu (`.igris/plugins/`)
...
igris uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z igris do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do igris.
...
:::note
Jeśli korzystasz z aplikacji komputerowej igris, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj igris czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do igris:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod igris. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://igris.ai/docs/sdk).
