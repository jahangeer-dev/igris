---
title: Serwer
description: Interakcja z serwerem igris za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `igris serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient igris.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `igris.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `igris` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `igris serve`, jak i `igris web`.
...
Kiedy uruchomisz `igris`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera igris do programowej interakcji z igris.
:::

Ta architektura umożliwia obsługę wielu klientów przez igris i programową interakcję z kodem otwartym.

Możesz uruchomić `igris serve`, aby uruchomić samodzielny serwer. Jeśli masz
igris TUI działa, `igris serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki igris [IDE](/docs/ide).
...
Serwer igris udostępnia następujące interfejsy API.
