...
- `.igris/plugins/` - Project-level plugins
- `~/.config/igris/plugins/` - Global plugins
...
**npm plugins** are installed automatically using Bun at startup. Packages and their dependencies are cached in `~/.cache/igris/node_modules/`.
...
1. Global config (`~/.config/igris/igris.json`)
2. Project config (`igris.json`)
3. Global plugin directory (`~/.config/igris/plugins/`)
4. Project plugin directory (`.igris/plugins/`)
...
- `client`: An igris SDK client for interacting with the AI.
...
Here are some examples of plugins you can use to extend igris.
...
Prevent igris from reading `.env` files:
...
Plugins can also add custom tools to igris:
...
The `tool` helper creates a custom tool that igris can call. It takes a Zod schema function and returns a tool definition with:

- `description`: What the tool does
- `args`: Zod schema for the tool's arguments
- `execute`: Function that runs when the tool is called

Your custom tools will be available to igris alongside built-in tools.
...
Levels: `debug`, `info`, `warn`, `error`. See [SDK documentation](https://igris.ai/docs/sdk) for details.
