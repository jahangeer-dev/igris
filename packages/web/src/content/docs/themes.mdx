...
By default, OpenCode uses our own `igris` theme.
...
2. **User config directory** - Defined in `~/.config/igris/themes/*.json` or `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.igris/themes/*.json`
4. **Current working directory** - Defined in `./.igris/themes/*.json`
