...
**npm 外掛**在啟動時使用 Bun 自動安裝。套件及其相依套件會快取在 `~/.cache/igris/node_modules/` 中。
...
1. 全域設定 (`~/.config/igris/igris.json`)
2. 專案設定 (`igris.json`)
3. 全域外掛目錄 (`~/.config/igris/plugins/`)
4. 專案外掛目錄 (`.igris/plugins/`)
...
日誌層級：`debug`、`info`、`warn`、`error`。詳情請參閱 [SDK 文件](https://igris.ai/docs/sdk)。
