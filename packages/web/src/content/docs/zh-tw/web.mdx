...
:::tip[Windows 使用者]
為獲得最佳體驗，建議從 [WSL](/docs/windows-wsl) 而非 PowerShell 執行 `igris web`。這可以確保正確的檔案系統存取和終端機整合。
:::
...
這會自動將主機名稱設定為 `0.0.0.0`，並將伺服器廣播為 `igris.local`。
...
使用者名稱預設為 `igris`，可以透過 `OPENCODE_SERVER_USERNAME` 進行更改。
