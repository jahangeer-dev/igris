...
:::tip[Windows Users]
For the best experience, run `igris web` from [WSL](/docs/windows-wsl) rather than PowerShell. This ensures proper file system access and terminal integration.
:::
...
This automatically sets the hostname to `0.0.0.0` and advertises the server as `igris.local`.
...
The username defaults to `igris` but can be changed with `OPENCODE_SERVER_USERNAME`.
...
You can also configure server settings in your `igris.json` config file:
