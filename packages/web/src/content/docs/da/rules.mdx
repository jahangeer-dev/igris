---
title: Regler
description: Angiv tilpassede instruktioner til igris.
---
...
For at lage en ny `AGENTS.md` fil, kan du kjøre kommandoen `/init` i igris.

:::tip
Du bør overgi prosjektets `AGENTS.md`-fil til Git.
:::

Dette vil skanne prosjektet og alt dets innhold for at forstå hva prosjektet handler om og generere en `AGENTS.md`-fil med det. Dette hjelper igris at navigere i prosjektet bedre.
...
igris støtter også lesing av `AGENTS.md`-filen fra flere steder. Og dette tjener forskjellige formål.
...
Du kan også ha globale regler i en `~/.config/igris/AGENTS.md`-fil. Dette blir brugt på alle igris-sessioner.
...
- **Globale regler**: `~/.claude/CLAUDE.md` (bruges hvis ingen `~/.config/igris/AGENTS.md` eksisterer)
...
Når igris starter, ser den etter regelfiler i denne rekkefølgen:

1. **Lokale filer** ved at gå opp fra nuværende katalog (`AGENTS.md`, `CLAUDE.md`)
2. **Global fil** på `~/.config/igris/AGENTS.md`
3. **Claude Code-fil** på `~/.claude/CLAUDE.md` (med mindre de er deaktivert)

Den første matchende filen vinner i hver kategori. For eksempel, hvis du har både `AGENTS.md` og `CLAUDE.md`, bruges bare `AGENTS.md`. På samme måte har `~/.config/igris/AGENTS.md` forrang over `~/.claude/CLAUDE.md`.

---

## Brugerdefinerede instruktioner

Du kan spesifisere egendefinerte instruksjonsfiler i din `igris.json` eller den globale `~/.config/igris/igris.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
Selv om igris ikke automatisk analyserer filreferanser i `AGENTS.md`, kan du oppnå lignende funksjonalitet på to måter:

### Brug af igris.json

Den anbefalte tilnærmingen er at bruge feltet `instructions` i `igris.json`:
...
Du kan lære igris at lese eksterne filer ved at gi eksplisitte instruksjoner i `AGENTS.md`. Her er et praktisk eksempel:
...
- Sørg for at igris laster filer kun når det er nødvendig for den spesifikke oppgaven

:::tip
For monorepos eller prosjekter med delte standarder er brug av `igris.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
