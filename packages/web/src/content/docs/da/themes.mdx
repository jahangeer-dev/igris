...
Som standard bruger OpenCode vårt eget `igris`-tema.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/igris/themes/*.json` eller `$XDG_CONFIG_HOME/igris/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.igris/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.igris/themes/*.json`
