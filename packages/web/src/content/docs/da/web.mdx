...
:::tip[Windows-brugere]
For den bedste oplevelse skal du køre `igris web` fra [WSL](/docs/windows-wsl) i stedet for PowerShell. Dette sikrer korrekt filsystemadgang og terminalintegration.
:::
...
Dette indstiller automatisk værtsnavnet til `0.0.0.0` og annoncerer serveren som `igris.local`.
...
Brugernavnet er som standard `igris`, men kan ændres med `OPENCODE_SERVER_USERNAME`.
...
Du kan også konfigurere serverindstillinger i din `igris.json`-konfigurationsfil:
