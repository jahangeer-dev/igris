...
1. **Fjernkonfiguration** (fra `.well-known/igris`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/igris/igris.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`igris.json` i projekt) - projektspecifikke indstillinger
5. **`.igris` mapper** - agent, kommandoer, plugins
...
:::note
`.igris` og `~/.config/igris` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/igris`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale OpenCode-konfiguration i `~/.config/igris/igris.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `igris.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
modes og plugins ligesom standard `.igris` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.igris` mapper, så den **kan tilsidesætte** deres indstillinger.

---

## Skema

Konfigurationsfilen har et skema, der defineres i [**`igris.ai/config.json`**](https://igris.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `igris serve` og `igris web` gennem indstillingen `server`.
...
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `igris.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/igris/agents/` eller `.igris/agents/`. [Learn more here](/docs/agents).
...
Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`igris run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/igris/commands/` eller `.igris/commands/`. [Learn more here](/docs/commands).
...
Som standard igris **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
Placer plugin-filer i `.igris/plugins/` eller `~/.config/igris/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
