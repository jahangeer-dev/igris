...
- `.igris/plugins/` - Plugins på projektniveau
- `~/.config/igris/plugins/` - Globale plugins
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/igris/node_modules/`.
...
1. Global konfiguration (`~/.config/igris/igris.json`)
2. Projektkonfiguration (`igris.json`)
3. Global plugin-mappe (`~/.config/igris/plugins/`)
4. Projekt plugin bibliotek (`.igris/plugins/`)
...
- `client`: En igris SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide igris.
...
Undgå igris i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til igris:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som igris kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for igris sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://igris.ai/docs/sdk) for detaljer.
