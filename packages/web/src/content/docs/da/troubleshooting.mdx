...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\igris\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `igris --log-level DEBUG`.

---

## Lagring

igris gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/igris/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\igris`
...
OpenCode Desktop kører en lokal OpenCode-server (`igris-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
- **macOS/Linux**: `~/.config/igris/igris.jsonc` (eller `~/.config/igris/igris.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/igris/igris.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\igris\igris.jsonc`
...
- **Globale plugins**
  ...
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\igris\plugins`
...
- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/igris`
- **Linux**: slet `~/.cache/igris` (eller kør `rm -rf ~/.cache/igris`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\igris`
...
Hvis `igris.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
- `igris.settings.dat` (skrivebordsstandardserver URL)
- `igris.global.dat` og `igris.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
3. Sørg for at du har den nyeste version med `igris upgrade`
...
For at finde ud af hvilke modeller du har adgang til, kør `igris models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\igris`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. igris installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\igris`

2. Start igris på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge igris til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
igris vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
