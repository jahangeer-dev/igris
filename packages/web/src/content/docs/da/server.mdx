---
title: Server
description: Interager med igris-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `igris serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en igris-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `igris.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `igris`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `igris serve` og `igris web`.
...
Når du kører `igris` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug igris-serveren til at interagere med igris programmatisk.
:::

Denne arkitektur lader igris understøtte flere klienter og lader dig interagere med igris programmatisk.

Du kan køre `igris serve` for at starte en selvstændig server. Hvis du har
igris TUI kørende, vil `igris serve` starte en ny server.
