...
Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/igris/auth.json`.
...
1. Kør kommandoen `/connect` i TUI, vælg igris og gå til [igris.ai/auth](https://igris.ai/auth).
...
1. Kør kommandoen `/connect` i TUI, vælg `OpenCode Go`, og gå til [igris.ai/auth](https://igris.ai/zen).
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører igris:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `igris.json`:
...
Du kan konfigurere igris til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at igris skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem igris-konfigurationen.
...
gemme tokenet i igris auth-lagring.
:::
...
`igris.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [igris-gitlab-auth](https://www.npmjs.com/package/igris-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `igris.json`:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem igris, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [igris-helicone-session](https://github.com/H2Shami/igris-helicone-session) plugin for automatisk at logge hver OpenCode-samtale som en session i Helicone.
...
Du kan konfigurere igris til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere igris til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere igris til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for OpenCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/igris) for detaljer.
:::
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem igris-konfigurationen.
...
5. Du kan også tilpasse dem gennem igris-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Du kan også tilpasse modeller gennem igris-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med igris. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `igris.json`-filen i projektkataloget:
...
1. **Tjek godkendelsesopsætningen**: Kør `igris auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek igris-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i igris-konfigurationen.
