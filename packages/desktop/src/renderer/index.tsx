...
import type { UpdaterState } from "@igris-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@igris-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.IGRIS_CHANNEL === "prod" &&
...
const deepLinkEvent = "igris:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__IGRIS__ ??= {}
  const pending = window.__IGRIS__.deepLinks ?? []
  window.__IGRIS__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `igris.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://igris.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("igris.global.dat").getItem("language")
