...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.igris.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `igris.settings.dat` is special: it maps to the `igris.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"igris.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // igris.settings.dat → the electron settings store ("igris.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "igris.global.dat").
  const storeName = filename === "igris.settings.dat" ? "igris.settings" : filename
