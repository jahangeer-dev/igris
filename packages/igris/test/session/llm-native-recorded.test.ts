...
import { LLMEvent, LLMResponse } from "@igris-ai/llm"
import { RequestExecutor } from "@igris-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.igris.ai/proxy/connections/${connection}/v1`
...
const RECORDED_SCENARIOS = [
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["igris", "native", "oauth", "tool-loop"],
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the igris LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      ...
      JSON.stringify({ $schema: "https://igris.ai/config.json", ...scenario.config(model) }),
    ),
  )
