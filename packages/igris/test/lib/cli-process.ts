// Subprocess test harness for the igris CLI. Spawns the real binary against
...
//
// Configuration flows through igris's built-in test affordances:
//   - IGRIS_CONFIG_CONTENT      : provider config inline, no files to find
//   - IGRIS_TEST_HOME           : pins os.homedir() → tmpdir
//   - IGRIS_DISABLE_PROJECT_CONFIG : skip walking up for igris.json
//   - IGRIS_PURE                : skip external plugin discovery + install
//   - IGRIS_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `igris.run` is fully wired. The shape supports adding more
// builders (`igris.serve(opts)`, `igris.acp(opts)`, `igris.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of IgrisCli.
...
import { AppProcess } from "@igris-ai/core/process"
...
const igrisRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(igrisRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    IGRIS_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    IGRIS_CONFIG_CONTENT: configJson,
    IGRIS_DISABLE_PROJECT_CONFIG: "1",
    IGRIS_PURE: "1",
    IGRIS_DISABLE_AUTOUPDATE: "1",
    IGRIS_DISABLE_AUTOCOMPACT: "1",
    IGRIS_DISABLE_MODELS_FETCH: "1",
    IGRIS_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `igris run`. New flags should
...
// `igris serve` is a long-lived process — it never exits on its own.
...
// `igris acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type IgrisCli = {
  ...
  // Spawn `igris serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `igris acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("igris.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          IGRIS_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("igris.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("igris.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "igris server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `igris serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("igris.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const igris: IgrisCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, igris })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `igris.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "igris") {
...
// every call site. Use this for any test that needs the igris CLI fixture.
//
...
// resources (e.g. `igris.serve`) without an extra `Effect.scoped` wrapper —
