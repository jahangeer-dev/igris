...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "igris.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "igris.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "igris.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "igris.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["igris.json", "igris.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "igris.json"),
        path.join(project, "igris.jsonc"),
      ])
    })
  })
