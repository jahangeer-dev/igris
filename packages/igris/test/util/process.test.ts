...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.IGRIS_TEST ?? "")'), {
      env: {
        IGRIS_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "IGRIS_TEST_SHELL"], {
      shell: true,
      env: {
        IGRIS_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("IGRIS_TEST_SHELL=ok")
  })
