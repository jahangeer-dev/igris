...
describe("igris read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // IGRIS_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["mcp", "list"])
        igris.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["providers", "list"])
        igris.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["models"])
        igris.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["agent", "list"])
        igris.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh IGRIS_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["session", "list"])
        igris.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["stats"])
        igris.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under IGRIS_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ igris }) =>
      Effect.gen(function* () {
        const r = yield* igris.spawn(["db", "path"])
        igris.expectExit(r, 0, "db path")
