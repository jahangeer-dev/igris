...
describe("igris acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ igris }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ igris }))
        ...
        expect(initialized.agentInfo?.name).toBe("Igris")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ igris })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("igris-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "igris-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.IGRIS_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("igris-login")
