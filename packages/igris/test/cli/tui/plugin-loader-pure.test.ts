...
`,
  ...
  const pure = process.env.IGRIS_PURE
  const meta = process.env.IGRIS_PLUGIN_META_FILE
  process.env.IGRIS_PURE = "1"
  process.env.IGRIS_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.IGRIS_PURE
    } else {
      process.env.IGRIS_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.IGRIS_PLUGIN_META_FILE
    } else {
      process.env.IGRIS_PLUGIN_META_FILE = meta
    }
  }
})
