...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ igris }) =>
    Effect.gen(function* () {
      const result = yield* igris.spawn(["--replay-limit", "10"])

      igris.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ igris }) =>
    Effect.gen(function* () {
      const result = yield* igris.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ igris }) =>
    Effect.gen(function* () {
      const result = yield* igris.spawn(["--mini", "--port", "4096"])
