// Subprocess integration tests for `igris run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `igris.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `IGRIS_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("igris run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("say hi")
        igris.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* igris.run("think", { extraArgs: ["--thinking"] })
        igris.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* igris.run("think again")
        igris.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("say hi", { format: "json" })
        igris.expectExit(result, 0)

        const events = igris.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ igris }) =>
      Effect.gen(function* () {
        const result = yield* igris.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        ...
        const result = yield* igris.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, igris }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* igris.run("request permission", { permission: { bash: "ask" } })
        igris.expectExit(denied, 0)
        ...
        const allowed = yield* igris.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        igris.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* igris.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        igris.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, igris }) =>
      Effect.gen(function* () {
        ...
        const server = yield* igris.serve()

        const result = yield* igris.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, igris }) =>
      Effect.gen(function* () {
        const result = yield* igris.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, igris }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* igris.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
