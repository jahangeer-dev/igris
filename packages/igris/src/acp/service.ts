...
export const AuthMethodID = "igris-login"
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `igris auth login` in the terminal",
      name: "Login with igris",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, igris provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const igrisProvider = providers[ProviderV2.ID.make("igris")]
  const igrisModel = igrisProvider ? Provider.sort(Object.values(igrisProvider.models))[0] : undefined
  if (igrisProvider && igrisModel) return { providerID: igrisProvider.id, modelID: igrisModel.id }
