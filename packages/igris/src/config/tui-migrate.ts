...
const TUI_SCHEMA_URL = "https://igris.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from igris.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const igris = await igrisFiles(input)
  for (const file of igris) {
...
async function igrisFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "igris"),
    ...(await Filesystem.findUp(["igris.json", "igris.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "igris"))
  }
  if (Flag.IGRIS_CONFIG) files.push(Flag.IGRIS_CONFIG)
