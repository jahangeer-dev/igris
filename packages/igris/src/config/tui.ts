...
import { isRecord } from "@igris-ai/tui/util/record"
...
import { TuiKeybind } from "@igris-ai/tui/config/keybind"
...
import { TuiConfig } from "@igris-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@igris/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old igris.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.igris`
  // folders between cwd and home, and IGRIS_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.IGRIS_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit IGRIS_TUI_CONFIG override, if set.
  if (Flag.IGRIS_TUI_CONFIG) {
    const configFile = Flag.IGRIS_TUI_CONFIG
  ...
  // 4. `.igris` directories (and IGRIS_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".igris") || dir === Flag.IGRIS_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".igris") && dir !== Flag.IGRIS_CONFIG_DIR) continue
