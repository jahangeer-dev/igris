...
const USER_AGENT = `igris/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const igrisProjectID = input.model.providerID.startsWith("igris")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("igris")
        ? {
            ...(igrisProjectID ? { "x-igris-project": igrisProjectID } : {}),
            "x-igris-session": input.sessionID,
            "x-igris-request": input.user.id,
            "x-igris-client": input.flags.client,
