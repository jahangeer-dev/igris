...
import type { LLMClientShape } from "@igris-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("igris"))
    return { type: "unsupported", reason: "provider is not openai, igris, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @igris-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains igris-owned. The native runtime only adapts
      // the @igris-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
