...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for igris serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(IgrisHttpApi)
...
export const PublicApi = IgrisHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "igris",
    version: "1.0.0",
    description: "igris api",
