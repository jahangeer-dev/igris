...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@igris-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@igris-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@igris-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@igris-ai/tui/context/theme"
import { Global } from "@igris-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@igris-ai/core/util/flock"
import { Flag } from "@igris-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@igris-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@igris-ai/core/v1/config/plugin"
import { createCommandShim } from "@igris-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@igris-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".igris"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".igris", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".igris", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.IGRIS_PURE ? [] : pluginOrigins
    if (Flag.IGRIS_PURE && pluginOrigins.length) {
    }
