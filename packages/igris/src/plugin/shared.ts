...
export const DEPRECATED_PLUGIN_PACKAGES = ["igris-openai-codex-auth", "igris-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, igrisVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(igrisVersion) || semver.major(igrisVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.igris
  if (typeof range !== "string") return
  if (!semver.satisfies(igrisVersion, range)) {
    throw new Error(`Plugin requires igris ${range} but running ${igrisVersion}`)
  }
}
