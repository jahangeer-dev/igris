import type { Hooks, PluginInput } from "@igris-ai/plugin"
import type { Model } from "@igris-ai/sdk/v2"
import { InstallationVersion } from "@igris-ai/core/installation/version"
import { OauthCallbackPage } from "@igris-ai/core/oauth/page"
...
async function listRouters(
  ...
  const res = await fetch(`${DO_GENAI_API}/models/routers`, {
    headers: {
      ...
      "User-Agent": `igris/${InstallationVersion}`,
    },
      ...
      methods: [
        {
          ...
          async authorize() {
            ...
            return {
              url,
              instructions:
                "Sign in to DigitalOcean in your browser. Igris CLI will use your DigitalOcean API token directly for inference and load your Inference Routers. Re-run /connect to refresh routers later.",
