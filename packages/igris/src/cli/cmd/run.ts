import type { PermissionV1 } from "@igris-ai/core/v1/permission"
import { FSUtil } from "@igris-ai/core/fs-util"
// CLI entry point for `igris run` and `igris --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`igris --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`igris --mini --attach`): connects to a running
//      igris server and runs interactive mode against it.
//
...
import { createIgrisClient, type IgrisClient, type ToolPart } from "@igris-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<IgrisClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run igris with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running igris server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to IGRIS_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to IGRIS_SERVER_USERNAME or 'igris')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: IgrisClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: IgrisClient, sessionID: string) {
      ...
      async function current(sdk: IgrisClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: IgrisClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: IgrisClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: IgrisClient) {
        ...
        async function loop(client: IgrisClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
