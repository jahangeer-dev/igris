...
Delete `packages/igris/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/igris/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/igris/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/igris`
- TUI plugin loader tests in `packages/igris` if runtime plugin API wiring changed
