...
- Author package entrypoint is `@igris-ai/plugin/tui`.
...
```json
{
  "$schema": "https://igris.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/igris-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@igris-ai/plugin/tui`.
- `@igris-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@igris-ai/plugin/tui"
...
- `igris plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `igris plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.igris` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.igris`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.igris`) is covered by regression tests.
...
That is what makes local config-scoped plugins able to import `@igris-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@igris-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"igris"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `igris.default`.
...
- Local plugins persist installed themes under the local `.igris/themes` area near the plugin config source.
...
- Local smoke plugin: `.igris/plugins/tui-smoke.tsx`
- Local vim plugin: `.igris/plugins/tui-vim.tsx`
- Local smoke config: `.igris/tui.json`
- Local smoke theme: `.igris/plugins/smoke-theme.json`
