...
Practical reference for a future `packages/server` split after the igris
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/igris`.
...
Until enough shared service code lives outside `packages/igris`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/igris`

It should not import `packages/igris` services while `packages/igris`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on igris-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/igris` implementation details.
