...
Trim `packages/igris/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/igris/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/igris`.
- `[x]` `bun typecheck` from `packages/igris`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/igris/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/igris`.
- `[x]` `bun dev generate > /tmp/igris-openapi.json` from `packages/igris`.
...
- Edit `packages/igris/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/igris-openapi.json` from `packages/igris`.
...
- Check source schemas in files like `packages/igris/src/session/schema.ts`, `packages/igris/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/igris/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/igris/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
