...
void (async () => {
  const events = await igris.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new igris session...")
    const { client, server } = igris
    ...
    console.log("✅ Created igris session:", createResult.data.id)
  ...
  console.log("📝 Sending to igris:", message.text)
