...
import { PromptInput } from "@igris-ai/schema/prompt-input"
import { Session } from "@igris-ai/schema/session"
import { Project } from "@igris-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@igris-ai/schema/schema"
...
import { SessionEvent } from "@igris-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Igris process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Igris process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
