...
> current private `@igris-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@igris-ai/ai`
...
- Secondary audience: Igris and other durable agent runtimes
...
import { LLM } from "@igris-ai/ai"
import { OpenAI } from "@igris-ai/ai/providers/openai"
...
```ts
import { Message } from "@igris-ai/ai"
...
import { LLM, Tool } from "@igris-ai/ai"
...
Igris and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@igris-ai/ai/promise"
import { OpenAI } from "@igris-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@igris-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@igris-ai/ai/provider"
...
@igris-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@igris-ai/ai/providers/openai
@igris-ai/ai/providers/anthropic
@igris-ai/ai/providers/google
...
```
...
Igris should migrate to `generateTurn` / `streamTurn`, preserving its durable
