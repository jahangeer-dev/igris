...
import { GoogleVertexPlugin } from "@igris-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.igris, (provider) => {
          provider.api = {
            ...
            url: "https://igris.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.igris))
