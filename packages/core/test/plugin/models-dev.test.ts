...
import { ModelsDevPlugin } from "@igris-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.IGRIS_MODELS_PATH,
          disabled: Flag.IGRIS_DISABLE_MODELS_FETCH,
        }
        Flag.IGRIS_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.IGRIS_MODELS_PATH = previous.path
          Flag.IGRIS_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
