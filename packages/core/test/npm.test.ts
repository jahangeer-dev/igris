...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@igris/acme")).toBe("@igris/acme")
    expect(Npm.sanitize("@igris/acme@1.0.0")).toBe("@igris/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/igris/acme.git"
    const expected = win ? "acme@git+https_//github.com/igris/acme.git" : spec
