...
import type { IntegrationOAuthMethodRegistration } from "@igris-ai/plugin/v2/effect/integration"
import { define } from "@igris-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.igris.ai"
const clientID = "igris-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    connected = (yield* ctx.integration.connection.active("igris")) !== undefined
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("igris")),
