...
export function fileLogger(file = path.join(Global.Path.log, "igris.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.IGRIS_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.IGRIS_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
