  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://igris.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://igris.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://igris.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://igris.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://igris.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://igris.example.com"),
      ),
    ).toEqual({
