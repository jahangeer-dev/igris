import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@igris-ai/sdk/v2/client"
...
import { Icon } from "@igris-ai/ui/icon"
import { IconButton } from "@igris-ai/ui/icon-button"
import { List, type ListRef } from "@igris-ai/ui/list"
import { ProviderIcon } from "@igris-ai/ui/provider-icon"
import { Spinner } from "@igris-ai/ui/spinner"
import { Tag } from "@igris-ai/ui/tag"
import { TextField } from "@igris-ai/ui/text-field"
import { ButtonV2 } from "@igris-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@igris-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@igris-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "igris-go") return language.t("dialog.provider.igrisGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "igris"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.igris.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["igris", "igris-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "igris" || provider.id === "igris-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "igris"
                                ? "dialog.provider.igris.tagline"
                                : "dialog.provider.igrisGo.tagline",
                            )}
                          </span>
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "igris"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.igrisZen.line1")}</div>
              <div>{language.t("provider.connect.igrisZen.line2")}</div>
              <div>
                {language.t("provider.connect.igrisZen.visit.prefix")}
                <Link
                  href="https://igris.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.igrisZen.visit.link")}
                </Link>
                {language.t("provider.connect.igrisZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "igris"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.igrisZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.igrisZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.igrisZen.visit.prefix")}
                <Link href="https://igris.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.igrisZen.visit.link")}
                </Link>
                {language.t("provider.connect.igrisZen.visit.suffix")}
              </div>
